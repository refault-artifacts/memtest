...
test_pattern_t test_list[NUM_TEST_PATTERNS] = {
	...
	{ true, ONE, 1, 6, 0, "DDR5 experiment" },
	{ false, SEQ, 1, 6, 0, "[Address test, own address in window]  " },
	{ false, SEQ, 2, 6, 0, "[Address test, own address + window]   " },
	{ false, PAR, 1, 6, 0, "[Moving inversions, 1s & 0s]           " },
	{ false, PAR, 1, 3, 0, "[Moving inversions, 8 bit pattern]     " },
	{ false, PAR, 1, 30, 0, "[Moving inversions, random pattern]    " },
#if TESTWORD_WIDTH > 32
	{ false, PAR, 1, 3, 0, "[Moving inversions, 64 bit pattern]    " },
#else
	{ false, PAR, 1, 3, 0, "[Moving inversions, 32 bit pattern]    " },
#endif
	{ false, PAR, 1, 81, 0, "[Block move]                           " },
	{ false, PAR, 1, 48, 0, "[Random number sequence]               " },
	{ false, PAR, 1, 6, 0, "[Modulo 20, random pattern]            " },
...
#define BARRIER                                                         \
	if (my_cpu >= 0) {                                              \
		if (TRACE_BARRIERS) {                                   \
			trace(my_cpu, "Run barrier wait at %s line %i", \
			      __FILE__, __LINE__);                      \
		}                                                       \
...
int run_test(int my_cpu, int test, int stage, int iterations)
{
	if (my_cpu == master_cpu) {
		if (window_num == 0) {
			// First window, so we need to test all selected lower
			// memory.
		...
		display_test_addresses(pb << 2, pe << 2,
				       num_pages_to_test << 2);
	}
	BARRIER;
	...
	case 3: {
		...
		BARRIER;
		ticks += test_mov_inv_fixed(my_cpu, iterations, pattern1,
					    pattern2);
		BAILOUT;

		BARRIER;
		ticks += test_mov_inv_fixed(my_cpu, iterations, pattern2,
					    pattern1);
		BAILOUT;
	} break;
	...
	case 4: {
		...
		for (int i = 0; i < 8; i++) {
			...
			BARRIER;
			ticks += test_mov_inv_fixed(my_cpu, iterations,
						    pattern1, pattern2);
			BAILOUT;
	...
	case 5:
		...
		for (int i = 0; i < iterations; i++) {
			...
			BARRIER;
			ticks += test_mov_inv_fixed(my_cpu, 2, pattern1,
						    pattern2);
			BAILOUT;
		}
		break;
	...
	case 6:
		for (int offset = 0; offset < TESTWORD_WIDTH; offset++) {
			BARRIER;
			ticks += test_mov_inv_walk1(my_cpu, iterations, offset,
						    false);
			BAILOUT;
	...
	case 9:
		...
		for (int i = 0; i < iterations; i++) {
			for (int offset = 0; offset < MODULO_N; offset++) {
				...
				BARRIER;
				ticks += test_modulo_n(my_cpu, 2, pattern1,
						       pattern2, MODULO_N,
						       offset);
				BAILOUT;

				BARRIER;
				ticks += test_modulo_n(my_cpu, 2, pattern2,
						       pattern1, MODULO_N,
						       offset);
				BAILOUT;
			}
