...
bool err_banner_redraw = false; // Redraw banner on new errors (if previsouly
				// removed)
...
static void parse_option(const char *option, const char *params)
{
	...
	if (strncmp(option, "console", 8) == 0) {
		...
		if (strncmp(params, "par", 4) == 0) {
			...
			cpu_mode = SEQ;
		} else if (strncmp(params, "rr", 3) == 0 ||
			   strncmp(params, "one", 4) == 0) {
			cpu_mode = ONE;
		}
...
static void display_initial_notice(void)
{
	if (smp_enabled) {
		display_notice(
			"Press <F1> to configure, <F2> to disable SMP, <Enter> to "
			"start testing");
	} else {
		display_notice(
			"Press <F1> to configure, <F2> to enable SMP, <Enter> to "
			"start testing ");
	}
}
	...
	for (int i = 0; i < pm_map_size; i++) {
		...
		if (pm_map[i].start < pm_limit_lower) {
			...
			if (pm_map[i].end > pm_limit_upper) {
				num_pages_to_test +=
					pm_limit_upper - pm_limit_lower;
			} else {
				num_pages_to_test +=
					pm_map[i].end - pm_limit_lower;
			}
			continue;
		}
		if (pm_map[i].end > pm_limit_upper) {
...
{
	if (n >= 0 && n < SEL_AREA) {
		printc(row + (n / SEL_W), POP_LM + (n % SEL_W),
		       enabled ? '*' : '.');
	}
}
...
static void address_range_menu(void)
{
	...
	printf(POP_R + 8, POP_LM, "Current range: %kB - %kB",
	       pm_limit_lower << 2, pm_limit_upper << 2);
	...
	while (!exit_menu) {
		...
		switch (get_key()) {
		case '1': {
			display_input_message(POP_R + 10,
					      "Enter lower limit: ");
			uintptr_t page = read_value(POP_R + 10, POP_LM + 19, 15,
						    -PAGE_SHIFT);
...
static bool set_all_cpus(cpu_state_t state, int display_offset)
{
	...
	for (int i = 1; i < num_available_cpus; i++) {
		cpu_state[i] = state;
		display_enabled(POP_R + 12, i - display_offset,
				state == CPU_STATE_ENABLED);
	}
	return true;
}
...
static void display_cpu_selection(int display_offset)
{
	...
	display_selection_header(POP_R + 10, num_available_cpus - 1,
				 display_offset);
	if (display_offset == 0) {
...
static void cpu_selection_menu(void)
{
	...
	while (!exit_menu) {
		...
		case '6':
			...
			prints(POP_R + 8, POP_LI + 6,
			       exclude_ecores ? "Exclude" : "Include");
			break;
		case 'u':
...
void config_init(void)
{
	...
	if (cmd_line_addr != 0) {
		...
		if (cmd_line_addr != 0) {
			parse_command_line((char *)cmd_line_addr,
					   cmd_line_size);
		}
	}
}
...
void config_menu(bool initial)
{
	...
	while (!exit_menu) {
		...
		if (initial) {
			if (!smp_enabled)
				set_foreground_colour(BOLD + BLACK);
			prints(POP_R + 7, POP_LI, "<F5>  CPU selection");
			if (!smp_enabled)
				set_foreground_colour(WHITE);
			if (no_temperature)
				set_foreground_colour(BOLD + BLACK);
			printf(POP_R + 8, POP_LI, "<F6>  Temperature %s",
			       enable_temperature ? "disable" : "enable ");
			if (no_temperature)
				set_foreground_colour(WHITE);
			printf(POP_R + 9, POP_LI, "<F7>  Boot trace %s",
			       enable_trace ? "disable" : "enable ");
