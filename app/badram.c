...
// New mask is 1 where both masks were 1 (b & d) and the addresses were equal
// ~(a ^ c). If addresses were unequal the new mask must be 0 to allow for both
// values.
 ...
 */
static void combine(uintptr_t addr1, uintptr_t mask1, uintptr_t addr2,
		    uintptr_t mask2, uintptr_t *addr, uintptr_t *mask)
{
	*mask = COMBINE_MASK(addr1, mask1, addr2, mask2);

	*addr = addr1 | addr2;
	*addr &= *mask; // Normalise to ensure sorting on .addr will work as
			// intended
}
 ...
 * with addr2/mask2.
 */
static uintptr_t combi_cost(uintptr_t addr1, uintptr_t mask1, uintptr_t addr2,
			    uintptr_t mask2)
{
...
static bool is_covered(pattern_t pattern)
{
	for (int i = 0; i < num_patterns; i++) {
		if (combi_cost(patterns[i].addr, patterns[i].mask, pattern.addr,
			       pattern.mask) == 0) {
			return true;
		}
	}
 ...
 * Assumes patterns is sorted by .addr asc and that for each index i, the
 * cheapest entry to merge with is at i-1 or i+1. Return -1 if <= 1 patterns
 * exist, else the index of the first entry of the pair (the other being that +
 * 1).
 */
static int cheapest_pair()
{
	// This is guaranteed to be overwritten with >= 0 as long as
	// num_patterns > 1
	int merge_idx = -1;
	...
	for (int i = 0; i < num_patterns - 1; i++) {
		uintptr_t tmp_cost =
			combi_cost(patterns[i].addr, patterns[i].mask,
				   patterns[i + 1].addr, patterns[i + 1].mask);
...
{
	pattern_t combined;
	combine(patterns[idx1].addr, patterns[idx1].mask, patterns[idx2].addr,
		patterns[idx2].mask, &combined.addr, &combined.mask);
	return combined;
}

/*
 * Insert pattern at index idx, shuffling other entries on index towards the
 * end.
 */
static void insert_at(pattern_t pattern, int idx)
{
	// Move all entries >= idx one index towards the end to make space for
	// the new entry
...
/*
 * Insert entry (addr, mask) in patterns in an index i so that
 * patterns[i-1].addr < patterns[i] NOTE: Assumes patterns is already sorted by
 * .addr asc!
 */
...
{
	pattern_t pattern = { .addr = addr, .mask = DEFAULT_MASK };
	...
	if (num_patterns > MAX_PATTERNS) {
		...
		// merge_idx will be -1 if num_patterns < 2, but that means
		// MAX_PATTERNS = 0 which is not a valid state anyway
		int merge_idx = cheapest_pair();

		pattern_t combined = combined_pattern(merge_idx, merge_idx + 1);

		// Remove the source pair so that we can maintain order as
		// combined does not necessarily belong in merge_idx
...
void badram_display(void)
{
	...
	for (int i = 0; i < num_patterns; i++) {
		...
		display_scrolled_message(col, "0x%0*x,0x%0*x", TESTWORD_DIGITS,
					 patterns[i].addr, TESTWORD_DIGITS,
					 patterns[i].mask);
		col += text_width;
	}
}
