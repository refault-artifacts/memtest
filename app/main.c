...
#define SHORT_BARRIER                                                       \
	if (TRACE_BARRIERS) {                                               \
		trace(my_cpu, "Start barrier wait at %s line %i", __FILE__, \
		      __LINE__);                                            \
	}                                                                   \
...
static void run_at(uintptr_t addr, int my_cpu)
{
	...
	if (my_cpu == 0) {
		...
		size_t locals_offset =
			_stacks - _start + BSP_STACK_SIZE - LOCALS_SIZE;
		for (int cpu_num = 0; cpu_num < num_available_cpus; cpu_num++) {
			memcpy((void *)(addr + locals_offset),
			       (void *)(_start + locals_offset), LOCALS_SIZE);
...
static bool set_load_addr(uintptr_t *load_addr, size_t program_size,
			  uintptr_t lower_limit, uintptr_t upper_limit)
{
	uintptr_t current_start = (uintptr_t)_start;
	if (current_start >= lower_limit &&
	    (current_start + program_size) <= upper_limit) {
...
static void global_init(void)
{
	...
	// Nothing before this should access the boot parameters, in case they
	// are located above 4GB. This is the first region we map, so it is
	// guaranteed not to fail.
	boot_params_addr =
		map_region(boot_params_addr, sizeof(boot_params_t), true);
	...
	// This may overwrite any data structures passed to us by the BIOS
	// and/or boot loader, e.g. the boot parameters, boot command line, and
	// ACPI tables. So do not access those data structures after this point.
	...
	size_t program_size = (_stacks - _start) + BSP_STACK_SIZE +
			      (num_enabled_cpus - 1) * AP_STACK_SIZE;

	bool load_addr_ok = set_load_addr(&low_load_addr, program_size, 0x1000,
					  LOW_LOAD_LIMIT) &&
			    set_load_addr(&high_load_addr, program_size,
					  LOW_LOAD_LIMIT, HIGH_LOAD_LIMIT);
	...
	for (int i = 0; i < pm_map_size; i++) {
		trace(0, "pm %0*x - %0*x", 2 * sizeof(uintptr_t),
		      pm_map[i].start, 2 * sizeof(uintptr_t), pm_map[i].end);
	}
	if (acpi_config.rsdp_addr != 0) {
		trace(0, "ACPI RSDP (v%u.%u) found in %s at %0*x",
		      acpi_config.ver_maj, acpi_config.ver_min, rsdp_source,
		      2 * sizeof(uintptr_t), acpi_config.rsdp_addr);
		trace(0, "ACPI FADT found at %0*x", 2 * sizeof(uintptr_t),
		      acpi_config.fadt_addr);
	}
	if (!load_addr_ok) {
...
static void setup_vm_map(uintptr_t win_start, uintptr_t win_end)
{
	...
	for (int i = 0; i < pm_map_size; i++) {
		...
		if (seg_start < seg_end && seg_start < win_end &&
		    seg_end > win_start) {
			num_mapped_pages += seg_end - seg_start;
			vm_map[vm_map_size].pm_base_addr = seg_start;
			vm_map[vm_map_size].start =
				first_word_mapping(seg_start);
			vm_map[vm_map_size].end = last_word_mapping(
				seg_end - 1, sizeof(testword_t));
			vm_map_size++;
		}
	}
}
	...
	do {
		...
		if (i_am_master) {
			if (window_num == 0 && test_list[test_num].stages > 1) {
				// A multi-stage test runs through all the
				// windows at each stage. Relocation may disrupt
				// the test.
				window_num = 1;
			}
			if (window_num == 0 &&
			    pm_limit_lower >= LOW_LOAD_LIMIT) {
		...
		if (window_num > 0) {
			...
			if (!dummy_run &&
			    (uintptr_t)&_start != high_load_addr) {
		...
		if (dummy_run) {
			if (i_am_master) {
				ticks_per_test[pass_num][test_num] += run_test(
					-1, test_num, test_stage, iterations);
			}
		} else {
			if (!map_window(vm_map[0].pm_base_addr)) {
				// Either there is no PAE or we are at the PAE
				// limit.
				break;
			}
...
void main(void)
{
	int my_cpu;
	if (init_state == 0) {
		// If this is the first time here, we must be CPU 0, as the APs
	...
	if (init_state < 2) {
		...
		if (my_cpu == 0) {
			...
			if (failed) {
				const char *message =
					"Failed to start CPU core %i. Press any "
					"key to reboot...";
				display_notice_with_args(strlen(message),
							 message, failed);
				while (get_key() == 0) {
				}
				reboot();
	...
	while (1)
		...
		// Due to the need to relocate ourselves in the middle of tests,
		// the following code cannot be written in the natural way as a
		// set of nested loops. So we have a single loop and use global
		// state variables to allow us to restart where we left off
	...
	while (1) {
		...
		if (dummy_run) {
			ticks_per_pass[pass_num] +=
				ticks_per_test[pass_num][test_num];
		}
