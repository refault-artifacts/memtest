...
void barrier_halt_wait(barrier_t *barrier)
{
	...
	waiting_flags[my_cpu].flag = true;
	//
	// There is a small window of opportunity for the wakeup signal to
	// arrive between us decrementing the barrier count and halting. So code
	// the following in assembler, both to ensure the window of opportunity
	// is as small as possible, and also to allow us to detect and skip over
	// the halt in the interrupt handler.
	//
	// if (__sync_sub_and_fetch(&barrier->count, 1) != 0) {
