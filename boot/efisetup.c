...
static efi_guid_t EFI_CONSOLE_OUT_DEVICE_GUID = { 0xd3b36f2c,
						  0xd551,
						  0x11d4,
						  { 0x9a, 0x46, 0x00, 0x90,
						    0x27, 0x3f, 0xc1, 0x4d } };
static efi_guid_t EFI_GRAPHICS_OUTPUT_PROTOCOL_GUID = {
	0x9042a9de,
	0x23dc,
	0x4a38,
	{ 0x96, 0xfb, 0x7a, 0xde, 0xd0, 0x80, 0x51, 0x6a }
};
static efi_guid_t EFI_LOADED_IMAGE_PROTOCOL_GUID = {
	0x5b1b31a1,
	0x9562,
	0x11d2,
...
#define efi_table_attr(table, attr) table->attr

#define efi_call_proto(proto, func, ...) proto->func(proto, ##__VA_ARGS__)

#define efi_call_bs(func, ...) sys_table->boot_services->func(__VA_ARGS__)

#define efi_call_rs(func, ...) sys_table->runtime_services->func(__VA_ARGS__)

#define efi_get_num_handles(size) (int)((size) / sizeof(efi_handle_t))

#define efi_get_handle_at(array, index) (array)[index]
...
static void wait_for_key(void)
{
	...
	while (efi_call_proto(efi_table_attr(sys_table, con_in),
			      read_key_stroke, &input_key) == EFI_NOT_READY) {
	}
}
...
static void test_frame_buffer(screen_info_t *si)
{
	...
	pixel_value.word = (r_value << si->red_pos) |
			   (g_value << si->green_pos) |
			   (b_value << si->blue_pos);
...
{
	// We only use ASCII characters in our command line options, so for
	// simplicity just truncate the command line if we find a non-ASCII
	// character.
	efi_char16_t *cmd_line = (efi_char16_t *)image->load_options;
	int max_length = image->load_options_size / sizeof(efi_char16_t);
	int length = 0;
	while (length < max_length && cmd_line[length] > 0x00 &&
	       cmd_line[length] < 0x80) {
		length++;
	}
	return length;
}
...
static efi_status_t alloc_memory(void **ptr, size_t size,
				 efi_phys_addr_t max_addr)
{
	efi_status_t status;
	...
	status = efi_call_bs(allocate_pages, EFI_ALLOCATE_MAX_ADDRESS,
			     EFI_LOADER_DATA, num_pages, &addr);
...
static efi_memory_desc_t *get_memory_desc(uintptr_t map_addr, size_t desc_size,
					  size_t n)
{
	return (efi_memory_desc_t *)(map_addr + n * desc_size);
}

static bool map_buffer_has_headroom(size_t buffer_size, size_t map_size,
				    size_t desc_size)
{
...
static efi_status_t
get_memory_map(efi_memory_desc_t **mem_map, uintn_t *mem_map_size,
	       uintn_t *mem_desc_size, uint32_t *mem_desc_version,
	       uintn_t *mem_map_key, uintn_t *map_buffer_size)
{
	...
	*map_buffer_size = *mem_map_size =
		32 * sizeof(efi_memory_desc_t); // for first try

again:
	status = efi_call_bs(allocate_pool, EFI_LOADER_DATA, *map_buffer_size,
			     (void **)mem_map);
	if (status != EFI_SUCCESS) {
		goto fail;
	}

	status = efi_call_bs(get_memory_map, mem_map_size, *mem_map,
			     mem_map_key, mem_desc_size, mem_desc_version);
	if (status == EFI_BUFFER_TOO_SMALL ||
	    !map_buffer_has_headroom(*map_buffer_size, *mem_map_size,
				     *mem_desc_size)) {
		efi_call_bs(free_pool, *mem_map);
		// Make sure there is some headroom so that the buffer can be
		// reused for a new map after allocations are no longer
		// permitted. It's unlikely that the map will grow to exceed
		// this headroom once we are ready to trigger
		// ExitBootServices().
...
static efi_graphics_output_t *find_gop(efi_handle_t *handles,
				       size_t handles_size)
{
	efi_status_t status;
	...
	for (int i = 0; i < efi_get_num_handles(handles_size); i++) {
		...
		status = efi_call_bs(handle_protocol, handle,
		...
		// Systems that use the UEFI Console Splitter may provide
		// multiple GOP devices, not all of which are backed by real
		// hardware. The workaround is to search for a GOP implementing
		// the ConOut protocol, and if one isn't found, to just fall
		// back to the first GOP.

		void *con_out = NULL;
		status = efi_call_bs(handle_protocol, handle,
				     &EFI_CONSOLE_OUT_DEVICE_GUID, &con_out);
		if (status == EFI_SUCCESS) {
#if DEBUG
			print_string(
				"This GOP implements the ConOut protocol\n");
#endif
			return gop;
		}
...
static efi_status_t set_screen_info_from_gop(screen_info_t *si,
					     ...
					     size_t handles_size)
{
	...
	for (uint32_t mode_num = 0; mode_num < mode->max_mode; mode_num++) {
		...
		uintn_t info_size;
		status = efi_call_proto(gop, query_mode, mode_num, &info_size,
					&info);
		if (status != EFI_SUCCESS) {
			continue;
		}
...
#endif
		get_bit_range(best_info.pixel_info.red_mask, &si->red_pos,
			      &si->red_size);
		get_bit_range(best_info.pixel_info.green_mask, &si->green_pos,
			      &si->green_size);
		get_bit_range(best_info.pixel_info.blue_mask, &si->blue_pos,
			      &si->blue_size);
		get_bit_range(best_info.pixel_info.rsvd_mask, &si->rsvd_pos,
			      &si->rsvd_size);
		si->lfb_depth = si->red_size + si->green_size + si->blue_size +
				si->rsvd_size;
		si->lfb_linelength =
			(best_info.pixels_per_scan_line * si->lfb_depth) / 8;
		break;
	default:
#if DEBUG
	...
	print_string(" R");
	print_dec(si->red_size);
	print_string(" G");
	print_dec(si->green_size);
	print_string(" B");
	print_dec(si->blue_size);
	print_string(" A");
	print_dec(si->rsvd_size);
	print_string("\n");
...
static efi_status_t set_screen_info(boot_params_t *boot_params)
{
	...
	status = efi_call_bs(locate_handle, EFI_LOCATE_BY_PROTOCOL,
			     &EFI_GRAPHICS_OUTPUT_PROTOCOL_GUID, NULL,
			     &handles_size, handles);
	if (status == EFI_BUFFER_TOO_SMALL) {
		status = efi_call_bs(allocate_pool, EFI_LOADER_DATA,
				     handles_size, (void **)&handles);
		if (status != EFI_SUCCESS) {
			return status;
		}
		status = efi_call_bs(locate_handle, EFI_LOCATE_BY_PROTOCOL,
				     ...
				     &handles_size, handles);
		if (status == EFI_SUCCESS) {
			status = set_screen_info_from_gop(
				&boot_params->screen_info, handles,
				handles_size);
		}
		if (status == EFI_NOT_FOUND) {
			// This may be a headless system. We can still output to
			// a serial console.
...
static efi_status_t
set_efi_info_and_exit_boot_services(efi_handle_t handle,
				    boot_params_t *boot_params)
{
	...
	status = get_memory_map(&mem_map, &mem_map_size, &mem_desc_size,
				&mem_desc_version, &mem_map_key,
				&map_buffer_size);
	if (status != EFI_SUCCESS) {
		goto fail;
	}
...
static void set_e820_map(boot_params_t *params)
{
	...
	for (size_t i = 0; i < num_descs && num_entries < E820_MAP_SIZE; i++) {
		efi_memory_desc_t *mem_desc =
			get_memory_desc(mem_map_addr, mem_desc_size, i);
		...
		if (prev && prev->type == e820_type &&
		    (prev->addr + prev->size) == mem_desc->phys_addr) {
			 ...
			 boot_params_t *boot_params)
{
	...
	if (boot_params == NULL) {
		...
		if (status != EFI_SUCCESS) {
			print_string(
				"failed to get handle for loaded image protocol\n");
			goto fail;
		}
		...
		status = alloc_memory((void **)&boot_params,
				      sizeof(boot_params_t) + cmd_line_length +
					      1,
				      0xbfffffff);
		if (status != EFI_SUCCESS) {
			print_string(
				"failed to allocate low memory for boot params\n");
			goto fail;
		}
		memset(boot_params, 0, sizeof(boot_params_t));

		uintptr_t cmd_line_addr =
			(uintptr_t)boot_params + sizeof(boot_params_t);
