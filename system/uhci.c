...
#define UHCI_LEGSUP_CLEAR                                               \
	(UHCI_LEGSUP_TBY60R | UHCI_LEGSUP_TBY60W | UHCI_LEGSUP_TBY64R | \
	 UHCI_LEGSUP_TBY64W | UHCI_LEGSUP_A20PTS)
...
#define UHCI_PORT_SC(n) \
	(io_base + 0x10 + 2 * n) // Port n Status and Control register (n =
				 // 0..MaxPortIdx)
...
#define MAX_UHCI_PORTS \
	8 // the UHCI spec. doesn't define this, so pick a sensible number
...
static bool reset_host_controller(uint16_t io_base)
{
	...
	usleep(1 * MILLISEC); // allow the controller some time to recover from
			      // reset
...
static bool reset_uhci_port(uint16_t io_base, int port_idx)
{
	...
	outw(port_status & ~UHCI_PORT_SC_PR, UHCI_PORT_SC(port_idx));
	return io_wait_until_clr(UHCI_PORT_SC(port_idx), UHCI_PORT_SC_PR,
				 5 * MILLISEC);
}
...
static bool enable_uhci_port(uint16_t io_base, int port_idx)
{
	...
	return io_wait_until_set(UHCI_PORT_SC(port_idx), UHCI_PORT_SC_PED,
				 1000 * MILLISEC);
}
...
static void disable_uhci_port(uint16_t io_base, int port_idx)
{
	uint16_t port_status = inw(UHCI_PORT_SC(port_idx));
	outw(port_status & ~UHCI_PORT_SC_PED, UHCI_PORT_SC(port_idx));
	(void)io_wait_until_clr(UHCI_PORT_SC(port_idx), UHCI_PORT_SC_PED,
				1000 * MILLISEC);
}

static void build_uhci_td(uhci_td_t *td, const usb_ep_t *ep, uint32_t pid,
			  uint32_t dt, uint32_t options, const void *buffer,
			  size_t length)
{
	uint32_t device_speed = (ep->device_speed == USB_SPEED_LOW) ?
					UHCI_TD_LOW_SPEED :
					UHCI_TD_FULL_SPEED;
	...
	if (options & UHCI_TD_IOC_Y) {
		...
		td->link_ptr = (uintptr_t)(td + 1) | UHCI_LP_TYPE_TD |
			       UHCI_LP_DEPTH_FIRST;
	}
	td->control_status = UHCI_TD_CERR(3) | device_speed | options |
			     UHCI_TD_ACTIVE;
	td->token = UHCI_TD_LENGTH(length) | dt |
		    UHCI_TD_ENDPOINT(ep->endpoint_num) |
		    UHCI_TD_DEVICE_ADDR(ep->device_id) | pid;
...
static uint16_t get_uhci_done(workspace_t *ws)
{
	...
	uint16_t status = inw(UHCI_USBSTS) &
			  (UHCI_USBSTS_INT | UHCI_USBSTS_ERR);
	if (status != 0) {
		if (status & UHCI_USBSTS_ERR ||
		    ws->qh[0].qe_link_ptr != UHCI_LP_TERMINATE) {
#if 1
			uintptr_t td_addr = ws->qh[0].qe_link_ptr & 0xfffffff0;
			uhci_td_t *td = (uhci_td_t *)td_addr;
			print_usb_info(
				" transfer failed TD %08x status %08x token %08x",
				td_addr, (uintptr_t)td->control_status,
				(uintptr_t)td->token);
#endif
			  ...
			  const usb_setup_pkt_t *setup_pkt)
{
	...
	write32(&ws->qh[0].qe_link_ptr, UHCI_LP_TERMINATE);
	build_uhci_td(&ws->td[0], ep, UHCI_TD_PID_SETUP, UHCI_TD_DT(0),
		      UHCI_TD_IOC_N, setup_pkt, sizeof(usb_setup_pkt_t));
	build_uhci_td(&ws->td[1], ep, UHCI_TD_PID_IN, UHCI_TD_DT(1),
		      UHCI_TD_IOC_Y, 0, 0);
	write32(&ws->qh[0].qe_link_ptr,
		(uintptr_t)(&ws->td[0]) | UHCI_LP_TYPE_TD);
			     ...
			     const void *buffer, size_t length)
{
	...
	write32(&ws->qh[0].qe_link_ptr, UHCI_LP_TERMINATE);
	int pkt_num = 0;
	build_uhci_td(&ws->td[pkt_num], ep, UHCI_TD_PID_SETUP,
		      UHCI_TD_DT(pkt_num & 1), UHCI_TD_IOC_N, setup_pkt,
		      sizeof(usb_setup_pkt_t));
	pkt_num++;
	while (length > packet_size) {
		build_uhci_td(&ws->td[pkt_num], ep, UHCI_TD_PID_IN,
			      UHCI_TD_DT(pkt_num & 1), UHCI_TD_SPD, buffer,
			      packet_size);
		pkt_num++;
	...
	build_uhci_td(&ws->td[pkt_num], ep, UHCI_TD_PID_IN,
		      UHCI_TD_DT(pkt_num & 1), UHCI_TD_IOC_N, buffer, length);
	pkt_num++;
	build_uhci_td(&ws->td[pkt_num], ep, UHCI_TD_PID_OUT, UHCI_TD_DT(1),
		      UHCI_TD_IOC_Y, 0, 0);
	write32(&ws->qh[0].qe_link_ptr,
		(uintptr_t)(&ws->td[0]) | UHCI_LP_TYPE_TD);
	return wait_for_uhci_done(ws);
...
static void poll_keyboards(const usb_hcd_t *hcd)
{
	...
	uint32_t status = inw(UHCI_USBSTS) &
			  (UHCI_USBSTS_INT | UHCI_USBSTS_ERR);
	if (status != 0) {
		outw(UHCI_USBSTS_INT | UHCI_USBSTS_ERR, UHCI_USBSTS);
		...
		for (int kbd_idx = 0; kbd_idx < ws->num_keyboards; kbd_idx++) {
			...
			uint32_t error_mask = UHCI_TD_STALLED | UHCI_TD_DB_ERR |
					      UHCI_TD_BABBLE | UHCI_TD_NAK_RX |
					      UHCI_TD_CRC_TO | UHCI_TD_BS_ERR;
			if (~status & error_mask) {
				...
				write32(&kbd_td->token,
					read32(&kbd_td->token) ^ UHCI_TD_DT(1));
			}

			// Reenable the TD.
			write32(&kbd_td->control_status,
				kbd_td->driver_data[0]);
			write32(&kbd_qh->qe_link_ptr,
				(uintptr_t)kbd_td | UHCI_LP_TYPE_TD);
		}
	}
}
...
bool uhci_reset(int bus, int dev, int func, uint16_t io_base)
{
	...
	if (!halt_host_controller(io_base))
		return false;
	if (!reset_host_controller(io_base))
		return false;
...
bool uhci_probe(uint16_t io_base, usb_hcd_t *hcd)
{
	...
	// Allocate the frame list. This needs to be aligned on a 4K page
	// boundary.
	uintptr_t fl_addr = heap_alloc(
		HEAP_TYPE_LM_1, UHCI_FL_LENGTH * sizeof(uint32_t), PAGE_SIZE);
	if (fl_addr == 0) {
	...
	for (int port_idx = 0; port_idx < root_hub.num_ports; port_idx++) {
		...
		uint16_t port_status = inw(UHCI_PORT_SC(port_idx));
		if ((~port_status & UHCI_PORT_SC_VALID) ||
		    port_status == 0xffff) {
		...
		if (~port_status & UHCI_PORT_SC_CCS)
			continue;

		// Reset the port.
		if (!reset_uhci_port(io_base, port_idx))
			continue;

		usleep(10 * MILLISEC); // USB reset recovery time

		// Enable the port.
		if (!enable_uhci_port(io_base, port_idx))
			continue;

		port_status = inw(UHCI_PORT_SC(port_idx));

		// Check the port is active.
		if (~port_status & UHCI_PORT_SC_PED)
			continue;

		num_devices++;

		// Get the port speed.
		usb_speed_t port_speed = port_status & UHCI_PORT_SC_LSDA ?
						 USB_SPEED_LOW :
						 USB_SPEED_FULL;
		...
		if (find_attached_usb_keyboards(
			    hcd, &root_hub, 1 + port_idx, port_speed,
	...
	// Initialise the interrupt QH and TD for each keyboard interface and
	...
	for (int kbd_idx = 0; kbd_idx < num_keyboards; kbd_idx++) {
		...
		build_uhci_td(kbd_td, kbd, UHCI_TD_PID_IN, UHCI_TD_DT(0),
			      UHCI_TD_IOC_Y, kbd_rpt, sizeof(hid_kbd_rpt_t));
