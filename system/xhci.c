...
typedef struct {
	...
	xhci_erst_entry_t erst[WS_ERST_SIZE]
		__attribute__((aligned(64))); // event ring segment table

	// Microcontroller data transfer ring
	ep_tr_t uc_tr;

	// Microcontroller data transfer buffer
	hid_uc_rpt_t uc_rpt;

	// Saved keyboard reports.
	hid_uc_rpt_t prev_uc_rpt;
	...
	// Microcontroller slot ID
	uint8_t uc_slot_id;

	// Microcontroller endpoint ID
	uint8_t uc_ep_id;
} workspace_t __attribute__((aligned(64)));

workspace_t *global_ws;
...
// The read64 and write64 functions provided here provide compatibility with
// both 32-bit and 64-bit hosts and with both 32-bit and 64-bit XHCI
// controllers.
...
{
	write32(&op_regs->usb_command,
		read32(&op_regs->usb_command) | XHCI_USBCMD_HCRST);

	usleep(1 * MILLISEC); // some controllers need time to recover from
			      // reset

	return wait_until_clr(&op_regs->usb_command, XHCI_USBCMD_HCRST,
			      1000 * MILLISEC) &&
	       wait_until_clr(&op_regs->usb_status, XHCI_USBSTS_CNR,
			      1000 * MILLISEC);
}
...
{
	write32(&op_regs->usb_command,
		read32(&op_regs->usb_command) | XHCI_USBCMD_R_S);
...
{
	return (read32(&op_regs->port_regs[port_idx].sc) & XHCI_PORT_SC_PS) >>
	       XHCI_PORT_SC_PS_OFFSET;
}

static bool reset_xhci_port(xhci_op_regs_t *op_regs, int port_idx)
{
	write32(&op_regs->port_regs[port_idx].sc,
		XHCI_PORT_SC_PP | XHCI_PORT_SC_PR | XHCI_PORT_SC_PRC);
	return wait_until_set(&op_regs->port_regs[port_idx].sc,
			      XHCI_PORT_SC_PRC, 1000 * MILLISEC);
}

static void disable_xhci_port(xhci_op_regs_t *op_regs, int port_idx)
{
	write32(&op_regs->port_regs[port_idx].sc,
		XHCI_PORT_SC_PP | XHCI_PORT_SC_PED);
	(void)wait_until_clr(&op_regs->port_regs[port_idx].sc, XHCI_PORT_SC_PED,
			     1000 * MILLISEC);
}
...
static void ring_device_doorbell(xhci_db_reg_t *db_regs, int slot_id,
				 int db_target)
{
...
static uint32_t enqueue_trb(xhci_trb_t *trb_ring, uint32_t ring_size,
			    uint32_t enqueue_state, uint32_t control,
			    uint64_t params1, uint32_t params2)
{
	// The ring enqueue state records the current cycle and the next free
	// slot.
	...
	if (index == (ring_size - 1)) {
		...
		write32(&trb_ring[index].control,
			XHCI_TRB_LINK | XHCI_TRB_TC | cycle);
		cycle ^= 1;
		index = 0;
	}
...
static void enqueue_xhci_command(workspace_t *ws, uint32_t control,
				 uint64_t params1, uint32_t params2)
{
	ws->cr_enqueue_state = enqueue_trb(ws->cr, WS_CR_SIZE,
					   ws->cr_enqueue_state, control,
					   params1, params2);
}

static bool get_xhci_event(workspace_t *ws, xhci_trb_t *event)
{
	// Get the event ring dequeue state, which records the current cycle and
...
static uint32_t wait_for_xhci_event(workspace_t *ws, uint32_t wanted_type,
				    int max_time, xhci_trb_t *event)
{
...
static void issue_setup_stage_trb(ep_tr_t *ep_tr,
				  const usb_setup_pkt_t *setup_pkt)
{
	uint64_t params1 = *(const uint64_t *)setup_pkt;
	uint32_t params2 = sizeof(usb_setup_pkt_t);
	uint32_t control = XHCI_TRB_SETUP_STAGE | XHCI_TRB_TRT_IN |
			   XHCI_TRB_IDT;
	ep_tr->enqueue_state = enqueue_trb(ep_tr->tr, EP_TR_SIZE,
					   ep_tr->enqueue_state, control,
					   params1, params2);
}

static void issue_data_stage_trb(ep_tr_t *ep_tr, const void *buffer,
				 uint32_t dir, size_t transfer_length)
{
...
static void issue_status_stage_trb(ep_tr_t *ep_tr, uint32_t dir)
{
	...
	ep_tr->enqueue_state = enqueue_trb(ep_tr->tr, EP_TR_SIZE,
					   ep_tr->enqueue_state, control, 0, 0);
}

static void issue_normal_trb(ep_tr_t *ep_tr, const void *buffer, uint32_t dir,
...
static bool setup_request(const usb_hcd_t *hcd, const usb_ep_t *ep,
			  const usb_setup_pkt_t *setup_pkt)
{
	...
	bool status = wait_for_xhci_event(ws, XHCI_TRB_TRANSFER_EVENT,
					  5000 * MILLISEC, &event);
	// print_usb_info("setup packet trb transfer ended with status code %i",
	//	       status);
	return (status == XHCI_EVENT_CC_SUCCESS);
}

static bool get_data_request(const usb_hcd_t *hcd, const usb_ep_t *ep,
			     const usb_setup_pkt_t *setup_pkt,
			     const void *buffer, size_t length)
{
	...
	issue_status_stage_trb(ep_tr, XHCI_TRB_DIR_OUT);
	ring_device_doorbell(ws->db_regs, ep->device_id, 1);
	return (wait_for_xhci_event(ws, XHCI_TRB_TRANSFER_EVENT,
				    5000 * MILLISEC,
				    &event) == XHCI_EVENT_CC_SUCCESS);
...
static int allocate_slot(const usb_hcd_t *hcd)
{
	...
	uintptr_t device_workspace_addr =
		heap_alloc(HEAP_TYPE_LM_1, DEVICE_WS_SIZE, PAGE_SIZE);
	...
	ws->control_ep_tr_addr =
		device_workspace_addr + XHCI_MAX_OP_CONTEXT_SIZE;
	ws->interrupt_ep_tr_addr = device_workspace_addr +
				   XHCI_MAX_OP_CONTEXT_SIZE + sizeof(ep_tr_t);

	// Allocate a device slot and set up its output context.

	enqueue_xhci_command(ws, XHCI_TRB_ENABLE_SLOT | XHCI_TRB_USB2_SLOT, 0,
			     0);
	ring_host_controller_doorbell(ws->db_regs);
	if (wait_for_xhci_event(ws, XHCI_TRB_COMMAND_COMPLETE, 100 * MILLISEC,
				&event) != XHCI_EVENT_CC_SUCCESS) {
		goto free_memory;
	}
...
static bool assign_address(const usb_hcd_t *hcd, const usb_hub_t *hub,
			   int port_num, usb_speed_t device_speed,
			   int device_id, usb_ep_t *ep0)
{
	workspace_t *ws = (workspace_t *)hcd->ws;
	...
	// Initialise the control endpoint descriptor. With the XHCI, we never
	// need to know the USB address of the device, so device_id actually
	...
	memset((xhci_input_context_t *)ws->input_context_addr, 0,
	       sizeof(xhci_input_context_t));

	xhci_ctrl_context_t *ctrl_context =
		(xhci_ctrl_context_t *)ws->input_context_addr;
	ctrl_context->add_context_flags = XHCI_CONTEXT_A(0) | XHCI_CONTEXT_A(1);

	xhci_slot_context_t *slot_context =
		(xhci_slot_context_t *)(ws->input_context_addr +
					ws->context_size);
	...
	if (hub->level > 0) {
		...
		usb_parent_t hs_parent =
			usb_hs_parent(hub, port_num, device_speed);
	...
	xhci_ep_context_t *ep_context =
		(xhci_ep_context_t *)(ws->input_context_addr +
				      2 * ws->context_size);
	...
	// Set the device address. For full speed devices we need to read the
	// first 8 bytes of the device descriptor to determine the maximum
	// packet size the device supports and update the device context
	// accordingly. For compatibility with some older USB devices we need to
	// read the first 8 bytes of the device descriptor before actually
	// setting the address. We can conveniently combine both these
	// requirements.
...
set_address:
	enqueue_xhci_command(
		ws, XHCI_TRB_ADDRESS_DEVICE | command_flags | device_id << 24,
		ws->input_context_addr, 0);
	ring_host_controller_doorbell(ws->db_regs);
	if (wait_for_xhci_event(ws, XHCI_TRB_COMMAND_COMPLETE, 5000 * MILLISEC,
				&event) != XHCI_EVENT_CC_SUCCESS) {
		return false;
	}
	if (command_flags == 0) {
		usleep(2 * MILLISEC + 1 * MILLISEC); // USB set address recovery
						     // time (plus a bit).
	}

	build_setup_packet(&setup_pkt, USB_REQ_FROM_DEVICE, USB_GET_DESCRIPTOR,
			   USB_DESC_DEVICE << 8, 0, fetch_length);
	if (!get_data_request(hcd, ep0, &setup_pkt, data_buffer,
			      fetch_length) ||
	...
	if (command_flags != 0) {
		...
		if (!valid_usb_max_packet_size(device->max_packet_size,
					       device_speed)) {
			return false;
		}
...
static bool configure_interrupt_endpoint(workspace_t *ws, const usb_ep_t *ep,
					 int hub_flag, int num_ports,
					 int tt_think_time, uintptr_t tr_addr,
					 size_t rpt_size)
{
	xhci_trb_t event;

	// Calculate the endpoint ID. This is used both to select an endpoint
	// context and as a doorbell target.
	int ep_id = 2 * ep->endpoint_num + 1; // EP <N> IN

	// The input context has already been initialised, so we just need to
	// change the values used by the CONFIGURE_ENDPOINT command before
	// issuing the command.

	xhci_ctrl_context_t *ctrl_context =
		(xhci_ctrl_context_t *)ws->input_context_addr;
	ctrl_context->add_context_flags = XHCI_CONTEXT_A(0) |
					  XHCI_CONTEXT_A(ep_id);

	xhci_slot_context_t *slot_context =
		(xhci_slot_context_t *)(ws->input_context_addr +
					ws->context_size);
	slot_context->params1 = ep_id << 27 | hub_flag << 26 |
				(slot_context->params1 & 0x00ffffff);
	slot_context->num_ports = num_ports;
	slot_context->params2 =
		ep->device_speed == USB_SPEED_HIGH ? tt_think_time : 0;

	xhci_ep_context_t *ep_context =
		(xhci_ep_context_t *)(ws->input_context_addr +
				      (1 + ep_id) * ws->context_size);
	ep_context->params1 = 0;
	ep_context->params2 = XHCI_EP_INTERRUPT_IN << 3 | 3 << 1; // EP Type |
								  // CErr
	...
	enqueue_xhci_command(ws,
			     XHCI_TRB_CONFIGURE_ENDPOINT | ep->device_id << 24,
			     ws->input_context_addr, 0);
	ring_host_controller_doorbell(ws->db_regs);
	uint32_t status = wait_for_xhci_event(ws, XHCI_TRB_COMMAND_COMPLETE,
					      100 * MILLISEC, &event);
	return (status == XHCI_EVENT_CC_SUCCESS);
}

static bool configure_hub_ep(const usb_hcd_t *hcd, const usb_ep_t *ep,
			     const usb_hub_t *hub)
{
	workspace_t *ws = (workspace_t *)hcd->ws;

	return configure_interrupt_endpoint(ws, ep, 1, hub->num_ports,
					    hub->tt_think_time,
					    ws->interrupt_ep_tr_addr,
					    (hub->num_ports + 7) / 8);
}

static bool configure_uc_ep(const usb_hcd_t *hcd, const usb_ep_t *ep)
{
	workspace_t *ws = (workspace_t *)hcd->ws;

	// Fill in the lookup tables in the workspace.
	ws->uc_slot_id = ep->device_id;
	// ws->uc_ep_id = 2 * ep->endpoint_num + 1; // EP <N> IN //convention
	ws->uc_ep_id = ep->endpoint_num * 2;

	// Configure the controller.
	// bool status = configure_interrupt_endpoint(
	//	ws, ep, 1, 0, 0, (uintptr_t)(&ws->uc_tr), sizeof(hid_uc_rpt_t));
	// return status;
	...
	// context and as a doorbell target.
	// int ep_id = 2 * ep->endpoint_num + 1; // EP <N> IN
	int ep_id = ep->endpoint_num * 2;

	// The input context has already been initialised, so we just need to
	// change the values used by the CONFIGURE_ENDPOINT command before
	// issuing the command.

	uintptr_t tr_addr = (uintptr_t)(&ws->uc_tr);
	uint32_t rpt_size = sizeof(hid_uc_rpt_t);
	...
	xhci_slot_context_t *slot_context =
		(xhci_slot_context_t *)(ws->input_context_addr +
					ws->context_size);
	slot_context->params1 = ep_id << 27 | 0 << 26 |
				(slot_context->params1 & 0x00ffffff);
	slot_context->num_ports = 1;
	slot_context->params2 = ep->device_speed == USB_SPEED_HIGH ? 0 : 0;

	xhci_ep_context_t *ep_context =
		(xhci_ep_context_t *)(ws->input_context_addr +
				      (1 + ep_id) * ws->context_size);
	ep_context->params1 = 0;
	ep_context->params2 = XHCI_EP_INTERRUPT_OUT << 3 | 3 << 1; // EP Type |
								   // CErr
	ep_context->interval = xhci_ep_interval(ep->interval, ep->device_speed);
	ep_context->max_burst_size = 0;
	ep_context->max_packet_size = ep->max_packet_size;
	...
	uint32_t status = wait_for_xhci_event(ws, XHCI_TRB_COMMAND_COMPLETE,
					      100 * MILLISEC, &event);
	print_usb_info("trb command completed with status code %i", status);
	if (status != XHCI_EVENT_CC_SUCCESS) {
		while (1)
			;
	}
	return (status == XHCI_EVENT_CC_SUCCESS);
...
bool send_data_uc_ep(workspace_t *ws, uint8_t *buf)
{
	issue_normal_trb(&ws->uc_tr, buf, XHCI_TRB_DIR_IN, 64);
	ring_device_doorbell(ws->db_regs, ws->uc_slot_id, ws->uc_ep_id);

	xhci_trb_t event;
	get_xhci_event(ws, &event);
	return true;
}
static void poll_keyboards(const usb_hcd_t *hcd)
{
	...
	while (get_xhci_event(ws, &event)) {
		if (event_type(&event) != XHCI_TRB_TRANSFER_EVENT ||
		    event_cc(&event) != XHCI_EVENT_CC_SUCCESS)
			continue;

		hid_uc_rpt_t *uc_rpt = &ws->uc_rpt;

		hid_uc_rpt_t *prev_uc_rpt = &ws->prev_uc_rpt;
		if (process_usb_keyboard_report(hcd, uc_rpt, prev_uc_rpt)) {
			*prev_uc_rpt = *uc_rpt;
		}

		ep_tr_t *uc_tr = &ws->uc_tr;
		issue_normal_trb(uc_tr, uc_rpt, XHCI_TRB_DIR_IN,
				 sizeof(hid_uc_rpt_t));
		ring_device_doorbell(ws->db_regs, ws->uc_slot_id, ws->uc_ep_id);
	}
#endif
...
static const hcd_methods_t methods = { .reset_root_hub_port =
					       reset_root_hub_port,
				       .allocate_slot = allocate_slot,
				       .release_slot = release_slot,
				       .assign_address = assign_address,
				       .configure_hub_ep = configure_hub_ep,
				       .configure_uc_ep = configure_uc_ep,
...
bool xhci_reset(uintptr_t base_addr)
{
	...
	while (ext_cap_offs != 0) {
		...
		if (ext_cap->id == XHCI_EXT_CAP_LEGACY_SUPPORT) {
			xhci_legacy_support_t *legacy_support =
				(xhci_legacy_support_t *)ext_cap_base;
	...
	xhci_op_regs_t *op_regs =
		(xhci_op_regs_t *)(base_addr + cap_regs->cap_length);

	// Ensure the controller is halted and then reset it.
	if (!halt_host_controller(op_regs))
		return false;
...
bool xhci_probe(uintptr_t base_addr, usb_hcd_t *hcd)
{
	...
	while (ext_cap_offs != 0) {
		...
		if (ext_cap->id == XHCI_EXT_CAP_SUPPORTED_PROTOCOL) {
			xhci_supported_protocol_t *protocol =
				(xhci_supported_protocol_t *)ext_cap_base;

#ifdef QEMU_WORKAROUND
			xhci_supported_protocol_t protocol_copy;
			memcpy32(&protocol_copy, protocol,
				 sizeof(protocol_copy));
			protocol = &protocol_copy;
#endif
			// Record the ports covered by this protocol.
			uint8_t protocol_type = protocol->params2 &
						0x1f; // the Protocol Slot Type
...
#if 1
			print_usb_info(
				"protocol revision %i.%i type %i offset %i count %i",
				protocol->revision_major,
				protocol->revision_minor / 16, protocol_type,
				protocol->port_offset, protocol->port_count);
#endif
			for (int i = 0; i < protocol->port_count; i++) {
				int port_idx = protocol->port_offset + i - 1;
				if (port_idx >= 0 &&
				    port_idx < XHCI_MAX_PORTS) {
	...
	xhci_op_regs_t *op_regs =
		(xhci_op_regs_t *)(base_addr + cap_regs->cap_length);
	xhci_rt_regs_t *rt_regs =
		(xhci_rt_regs_t *)(base_addr + cap_regs->rts_offset);
	xhci_db_reg_t *db_regs =
		(xhci_db_reg_t *)(base_addr + cap_regs->db_offset);
	...
	uintptr_t num_scratchpad_buffers =
		((cap_regs->hcs_params2 >> 21) & 0x1f) << 5 |
		((cap_regs->hcs_params2 >> 27) & 0x1f);

	// Allocate and clear the scratchpad memory on the heap. This must be
	// aligned to the controller page size.
	uintptr_t scratchpad_size = num_scratchpad_buffers * xhci_page_size;
	uintptr_t scratchpad_paddr =
		heap_alloc(HEAP_TYPE_HM_1, scratchpad_size, xhci_page_size);
	if (scratchpad_paddr == 0) {
		goto no_devices_found;
	}
	uintptr_t scratchpad_vaddr =
		map_region(scratchpad_paddr, scratchpad_size, true);
	...
	// Allocate and initialise the device context base address and
	// scratchpad buffer arrays on the heap. Both need to be aligned on a 64
	// byte boundary.
	uintptr_t device_context_index_size =
		(1 + max_slots) * sizeof(uint64_t);
	uintptr_t scratchpad_buffer_index_offs =
		round_up(device_context_index_size, 64);
	uintptr_t scratchpad_buffer_index_size =
		num_scratchpad_buffers * sizeof(uint64_t);
	uintptr_t device_context_index_paddr = heap_alloc(
		HEAP_TYPE_HM_1,
		scratchpad_buffer_index_offs + scratchpad_buffer_index_size,
		64);
	if (device_context_index_paddr == 0) {
		goto no_devices_found;
	}
	uintptr_t device_context_index_vaddr = map_region(
		device_context_index_paddr,
		scratchpad_buffer_index_offs + scratchpad_buffer_index_size,
		true);
	...
	memset((void *)device_context_index_vaddr, 0,
	       device_context_index_size);

	uint64_t *device_context_index = (uint64_t *)device_context_index_vaddr;
	if (num_scratchpad_buffers > 0) {
		uintptr_t scratchpad_buffer_index_paddr =
			device_context_index_paddr +
			scratchpad_buffer_index_offs;
		uintptr_t scratchpad_buffer_index_vaddr =
			device_context_index_vaddr +
			scratchpad_buffer_index_offs;
		device_context_index[0] = scratchpad_buffer_index_paddr;
		uint64_t *scratchpad_buffer_index =
			(uint64_t *)scratchpad_buffer_index_vaddr;
		for (uintptr_t i = 0; i < num_scratchpad_buffers; i++) {
			scratchpad_buffer_index[i] =
				scratchpad_paddr + i * xhci_page_size;
		}
	}

	// Allocate and initialise a workspace for this controller. This needs
	// to be permanently mapped into virtual memory.
	uintptr_t workspace_addr =
		heap_alloc(HEAP_TYPE_LM_1, sizeof(workspace_t), PAGE_SIZE);
	if (workspace_addr == 0) {
		goto no_devices_found;
	}
	...
	// Allocate and initialise the input context data structure. This needs
	// to be contained within a single page.
	ws->input_context_addr =
		heap_alloc(HEAP_TYPE_LM_1, XHCI_MAX_IP_CONTEXT_SIZE, PAGE_SIZE);
	...
	// Initialise the ERST for the primary interrupter. We only use the
	// first segment.
	...
	write64(&op_regs->cr_control, (read64(&op_regs->cr_control) & 0x30) |
					      (uintptr_t)(&ws->cr) | 0x1);
	write64(&op_regs->dcbaap, device_context_index_paddr);
	write32(&op_regs->config,
		(read32(&op_regs->config) & 0xfffffc00) | max_slots);
	...
	// Scan the ports, looking for hubs and microcontrollers.
	usb_ep_t microcontroller;

	int num_devices = 0;
	bool microcontroller_found = false;

	for (int port_idx = 0; port_idx < root_hub.num_ports; port_idx++) {
		// We only expect to find microcontrollers on USB2 ports.
		...
		if (~port_status & XHCI_PORT_SC_CCS)
			continue;

		print_usb_info("device is connected to root port %i", port_idx);

		// Reset the port.
		if (!reset_xhci_port(op_regs, port_idx)) {
			print_usb_info("could not reset port %i", port_idx);
			continue;
		}
		...
		if (~port_status & XHCI_PORT_SC_CCS) {
			print_usb_info("port %i is not active", port_idx);
			continue;
		}
		if (~port_status & XHCI_PORT_SC_PED) {
			print_usb_info("port %i is not active", port_idx);
			continue;
		}

		// Now the port has been enabled, we can determine the device
		// speed.
		usb_speed_t device_speed = xhci_to_usb_speed(
			get_xhci_device_speed(ws->op_regs, port_idx));
		...
		// Look for microcontrollers attached directly or indirectly to
		// this port.
		if (find_attached_usb_microcontroller(
			    hcd, &root_hub, 1 + port_idx, device_speed, slot_id,
			    &num_devices, microcontroller)) {
			microcontroller_found = true;
			break;
		}

		// If we didn't find any microcontroller interfaces, we disable
		// the port and free the slot.
		disable_xhci_port(op_regs, port_idx);
		release_slot(hcd, slot_id);
	}

	print_usb_info(" Found %i device%s, %i microcontroller%s", num_devices,
		       num_devices != 1 ? "s" : "", microcontroller_found,
		       microcontroller_found ? "" : "s");

	if (!microcontroller_found) {
		(void)halt_host_controller(op_regs);
		goto no_devices_found;
	}

	// Initialise the interrupt TRB ring for the microcontroller.
	ep_tr_t *uc_tr = &ws->uc_tr;
	uc_tr->enqueue_state = EP_TR_SIZE; // cycle = 1, index = 0

	hid_uc_rpt_t *uc_rpt = &ws->uc_rpt;

	// startup packet
	uint8_t buf[64] = { 0xAA, 0xAA, 0xAA, 0xAA, 0xAA };
	memset(buf + 5, 0x00, 64 - 5);
	// send_data_uc_ep(ws, buf);
	global_ws = ws;

	// issue_normal_trb(uc_tr, buf, XHCI_TRB_DIR_OUT, 64);
	// ring_device_doorbell(ws->db_regs, ws->uc_slot_id, ws->uc_ep_id);
	// print_usb_info("uc_slot_id: %i, uc_ep_id: %i", ws->uc_slot_id,
	//	       ws->uc_ep_id);
