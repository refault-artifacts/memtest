...
typedef enum { CORE_UNKNOWN, CORE_PCORE, CORE_ECORE } core_type_t;
...
#define CPUID_VENDOR_STR_LENGTH \
	(CPUID_VENDOR_LENGTH * sizeof(uint32_t) + 1) // includes space for null
						     // terminator
...
#define CPUID_BRAND_STR_LENGTH \
	(CPUID_BRAND_LENGTH * sizeof(uint32_t) + 1) // includes space for null
						    // terminator
 ...
 */
static inline void cpuid(uint32_t op, uint32_t count, uint32_t *eax,
