...
static void read_sku(char *sku, uint8_t slot_idx, uint16_t offset,
		     uint8_t max_len);
...
char buf[64];
bool spdi_data_sent = false;

static void save_spdi(spd_info spdi)
{
	/* initialize buffer to zero */
	memset(DIMM_sku, 0x00, 64);
	memset(DIMM_man, 0x00, 32);
	memset(DIMM_type, 0x00, 8);
	DIMM_year = 0;
	DIMM_week = 0;

	/* DDR type */
	memcpy(DIMM_type, spdi.type, 4);

	/* Print Manufacturer from JEDEC106 */
	for (uint32_t i = 0; i < JEP106_CNT; i++) {
		if (spdi.jedec_code == jep106[i].jedec_code) {
			memcpy(DIMM_man, jep106[i].name,
			       strlen(jep106[i].name));
			break;
		}
	}

	DIMM_size = spdi.module_size * 1024;

	/* Print model information */
	memcpy(DIMM_sku, spdi.sku, 33);

	DIMM_week = spdi.fab_week;
	DIMM_year = spdi.fab_year;
}

static void send_spdi(spd_info spdi)
{
	/* this is a DIMM info packet */
	buf[0] = 0xBB;
	buf[1] = 0x13;
	/* initialize buffer to zero */
	...
	/* DDR type */
	memcpy(buf + 2, spdi.type, 4);

	/* Print Manufacturer from JEDEC106 */
	for (uint32_t i = 0; i < JEP106_CNT; i++) {
		if (spdi.jedec_code == jep106[i].jedec_code) {
			memcpy(buf + 10, jep106[i].name,
			       strlen(jep106[i].name));
			break;
		}
	}
	...
	/* Print model information */
	memcpy(buf + 28, spdi.sku, 33);

	buf[62] = spdi.fab_week;
	buf[63] = spdi.fab_year;
...
static void print_spdi(spd_info spdi, uint8_t lidx)
{
	...
	// Print Slot Index, Module Size, type & Max frequency (Jedec or
	// XMP)
	curcol = printf(LINE_SPD + lidx, 0, "DIMM %i: %kB %s-%i", spdi.slot_num,
			spdi.module_size * 1024, spdi.type, spdi.freq);
	...
	for (i = 0; i < JEP106_CNT; i++) {
		if (spdi.jedec_code == jep106[i].jedec_code) {
			curcol = printf(LINE_SPD + lidx, ++curcol, "- %s",
					jep106[i].name);
			break;
		}
	}
	...
	if (spdi.jedec_code == 0) {
		...
		curcol = printf(LINE_SPD + lidx, ++curcol, "- Unknown (0x%x)",
				spdi.jedec_code);
	}
	...
	if (curcol <= 69 && spdi.fab_week <= 53 && spdi.fab_week != 0 &&
	    (spdi.fab_year < 32 ||
	     (spdi.fab_year >= 96 && spdi.fab_year <= 99))) {
		curcol = printf(LINE_SPD + lidx, ++curcol, "(%02i%02i-W%02i)",
				spdi.fab_year >= 96 ? 19 : 20, spdi.fab_year,
				spdi.fab_week);
	}
...
static void parse_spd_ddr5(spd_info *spdi, uint8_t slot_idx)
{
	...
	spdi->XMP = ((get_spd(slot_idx, 640) == 0x0C &&
		      get_spd(slot_idx, 641) == 0x4A)) ?
			    3 :
			    0;
	...
	if (spdi->XMP == 3) {
		...
		// JEDEC
		tCK = get_spd(slot_idx, 21) << 8 | get_spd(slot_idx, 20);
	}
	...
	if (spdi->XMP == 3) {
		...
		spdi->tCL += spdi->tCL % 2; // if tCL is odd, round to
					    // upper even.
...
static void parse_spd_ddr4(spd_info *spdi, uint8_t slot_idx)
{
	...
	spdi->module_size =
		1U << (((get_spd(slot_idx, 4) & 0xF) + 5) + // Total SDRAM
							    // capacity:
							    // (256 Mbits
							    // <<
							    // byte4[3:0]
							    ...
							    // values
							    // >= 8) / 1 KB
		       ((get_spd(slot_idx, 13) & 0x7) + 3) - // Primary Bus
							     // Width: 8 <<
							     // byte13[2:0]
		       ((get_spd(slot_idx, 12) & 0x7) + 2) + // SDRAM
							     // Device
							     // Width: 4 <<
							     // byte12[2:0]
		       ((get_spd(slot_idx, 12) >> 3) & 0x7) + // Number of
							      // Ranks:
							      // byte12[5:3]
		       ((get_spd(slot_idx, 6) >> 4) & 0x7) // Die count -
							   // 1: byte6[6:4]
		);
...
static void parse_spd_ddr3(spd_info *spdi, uint8_t slot_idx)
{
	...
	spdi->module_size =
		1U << (((get_spd(slot_idx, 4) & 0xF) + 5) + // Total SDRAM
							    ...
							    // byte4[3:0])
							    // / 1 KB
		       ((get_spd(slot_idx, 8) & 0x7) + 3) - // Primary Bus
							    // Width: 8 <<
							    // byte8[2:0]
		       ((get_spd(slot_idx, 7) & 0x7) + 2) // SDRAM Device
							  // Width: 4 <<
							  // byte7[2:0]
		);

	spdi->module_size *= ((get_spd(slot_idx, 7) >> 3) & 0x7) +
			     1; // Number of Ranks: byte7[5:3]
...
static void parse_spd_ddr(spd_info *spdi, uint8_t slot_idx)
{
	...
	spdi->tRAS = (uint16_t)((float)get_spd(slot_idx, 30) / tckns +
				ROUNDING_FACTOR);
...
static void parse_spd_sdram(spd_info *spdi, uint8_t slot_idx)
{
	...
	uint8_t spd_byte3 = get_spd(slot_idx, 3) & 0x0F; // Number of
							 // Row
							 // Addresses (2
							 // x 4 bits,
							 // upper part
							 ...
							 // banking
							 // used)
	uint8_t spd_byte4 = get_spd(slot_idx, 4) & 0x0F; // Number of
							 // Column
							 // Addresses (2
							 // x 4 bits,
							 // upper part
							 ...
							 // banking
							 // used)
	uint8_t spd_byte5 = get_spd(slot_idx, 5); // Number of Banks on
						  // module (8 bits)
	uint8_t spd_byte17 = get_spd(slot_idx,
				     17); // SDRAM Device Attributes,
					  // Number of Banks on the
					  // discrete SDRAM Device (8
					  // bits)

	// Size in MB
	if ((spd_byte3 != 0) && (spd_byte4 != 0) &&
	    (spd_byte3 + spd_byte4 > 17) && (spd_byte3 + spd_byte4 <= 29) &&
	    (spd_byte5 <= 8) && (spd_byte17 <= 8)) {
		spdi->module_size = (1U << (spd_byte3 + spd_byte4 - 17)) *
				    ((uint16_t)spd_byte5 * spd_byte17);
	} else {
		spdi->module_size = 0;
	}
	...
	spdi->tRAS =
		(uint16_t)(get_spd(slot_idx, 30) / tckns + ROUNDING_FACTOR);
...
static bool find_in_did_array(uint16_t did, const uint16_t *ids,
			      unsigned int size)
{
...
static bool find_smb_controller(uint16_t vid, uint16_t did)
{
	...
	switch (vid) {
	case PCI_VID_INTEL: {
		if (find_in_did_array(did, intel_ich5_dids,
				      sizeof(intel_ich5_dids) /
					      sizeof(intel_ich5_dids[0]))) {
			return ich5_get_smb();
		}
	...
	case PCI_VID_SIS:
		switch (did) {
		// case 0x0008:
		// SiS5595, SiS630 or other SMBus controllers - it's
		// complicated. case 0x0016: SiS961/2/3, known as
		// "SiS96x" SMBus controllers. case 0x0018: case 0x0964:
	...
	case PCI_VID_VIA:
		...
		// Try SMB base address = 0x90, then SMB base address =
		// 0x80 viapro SMBus controller, i.e. PIIX4 with a small
		// quirk. case 0x3051: // 82C596B_3
		...
		case 0x3177: // 8235
			...
			// viapro I2C controller, i.e. PIIX4 with a
			// small quirk.
	...
	case PCI_VID_ALI:
		...
		// nearly compatible with PIIX4 according to Linux
		// i2c-ali1563 driver. case 0x7101: // ali1535 (M1535)
		// or ali15x3 (M1533/M1543) SMBus controllers
		default:
			return false;
		}
		break;
	...
	case PCI_VID_SERVERWORKS:
		switch (did) {
		case 0x0201: // CSB5
			// From Linux i2c-piix4 driver: unlike its
			// siblings, this model needs a quirk.
...
static bool amd_sb_get_smb(void)
{
	...
	if ((smbus_id & 0xFFFF) == 0x4385 && rev_id <= 0x3D) {
		// Older AMD SouthBridge (SB700 & older) use PIIX4
		// registers
		return piix4_get_smb(PIIX4_SMB_BASE_ADR_DEFAULT);
		...
		// AMD SB (SB800 up to pre-FT3/FP4/AM4) uses specific
		// registers
...
static bool fch_zen_get_smb(void)
{
	...
	// Special case for AMD Family 19h & Extended Model > 4 (get smb
	// address in memory)
	if ((imc_type == IMC_K19_CZN || imc_type == IMC_K19_RPL) &&
	    pm_reg == 0xFFFF) {
		smbusbase =
			((*(const uint32_t *)(0xFED80000 + 0x300) >> 8) & 0xFF)
			<< 8;
		return true;
	}
...
static bool nv_mcp_get_smb(void)
{
	...
	uint16_t x = pci_config_read16(0, smbdev, smbfun, smbus_base_adr) &
		     0xFFFC;
...
***************************** /
************************************************************************************
/   Be absolutely sure to know what you're doing before changing the
function below! /       You can easily WRITE into the SPD (especially
with DDR5) and corrupt it
