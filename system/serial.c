...
static void serial_write_reg(struct serial_port *port, uint16_t reg,
			     uint8_t val)
{
	union {
		uintptr_t addr;
...
void tty_init(void)
{
	...
	serial_write_reg(&console_serial, UART_LCR,
			 uart_status | UART_LCR_DLAB);
	...
	serial_write_reg(&console_serial, UART_LCR, lcr); /* No parity, 8 data
							     bits, 1 stop */
	serial_div = (console_serial.refclk / console_serial.baudrate) / 16;
	serial_write_reg(&console_serial, UART_LCR, 0x80 | lcr); /* Access
								    divisor
								    latch */
	serial_write_reg(&console_serial, UART_DLL, serial_div & 0xff); /* baud
									   rate
									   divisor
									 */
	...
	serial_write_reg(&console_serial, UART_IER, 0x00); /* Disable all
							      interrupts */
...
void tty_send_region(int start_row, int start_col, int end_row, int end_col)
{
	...
	for (int row = start_row; row <= end_row; row++) {
		// Always use absolute positioning instead of relying on CR-LF
		// to avoid issues when a CR-LF is lost (especially with
		// Industrial RS232/Ethernet converters).
		tty_goto(row, start_col);

		// Copy Shadow buffer to TTY buffer
		pos = 0;
		for (int col = start_col; col <= end_col; col++) {
			inverse = ((shadow_buffer[row][col].attr & 0x70) >> 4 !=
				   BLACK);
