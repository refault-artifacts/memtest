...
void cpuid_init(void)
{
	...
	cpuid(0x0, 0, &cpuid_info.max_cpuid, &cpuid_info.vendor_id.raw[0],
	      &cpuid_info.vendor_id.raw[2], &cpuid_info.vendor_id.raw[1]);
	...
	if (cpuid_info.max_cpuid >= 1) {
		cpuid(0x1, 0, &cpuid_info.version.raw,
		      &cpuid_info.proc_info.raw, &cpuid_info.flags.raw[1],
		      &cpuid_info.flags.raw[0]);
	}

	// Get the digital thermal sensor & power management status bits.
	if (cpuid_info.max_cpuid >= 6) {
		cpuid(0x6, 0, &cpuid_info.dts_pmp, &reg[0], &reg[1], &reg[2]);
	}

	// Get the max extended cpuid.
	cpuid(0x80000000, 0, &cpuid_info.max_xcpuid, &reg[0], &reg[1], &reg[2]);

	// Get extended feature flags, only save EDX.
	if (cpuid_info.max_xcpuid >= 0x80000001) {
		cpuid(0x80000001, 0, &reg[0], &reg[1], &reg[2],
		      &cpuid_info.flags.raw[2]);
	}

	// Get the brand ID.
	if (cpuid_info.max_xcpuid >= 0x80000004) {
		cpuid(0x80000002, 0, &cpuid_info.brand_id.raw[0],
		      &cpuid_info.brand_id.raw[1], &cpuid_info.brand_id.raw[2],
		      &cpuid_info.brand_id.raw[3]);
		cpuid(0x80000003, 0, &cpuid_info.brand_id.raw[4],
		      &cpuid_info.brand_id.raw[5], &cpuid_info.brand_id.raw[6],
		      &cpuid_info.brand_id.raw[7]);
		cpuid(0x80000004, 0, &cpuid_info.brand_id.raw[8],
		      &cpuid_info.brand_id.raw[9], &cpuid_info.brand_id.raw[10],
		...
		if (cpuid_info.max_xcpuid >= 0x80000008) {
			...
			if (cpuid_info.max_xcpuid >= 0x8000001E) {
				...
				if (cpuid_info.version.extendedFamily >= 8) {
					...
					cpuid_info.flags.htt = 0; // Pre-ZEN
								  // never has
								  // SMT
				}
			}
			cpuid_info.topology.core_count =
	...
	case 'G':
		...
		if (cpuid_info.max_cpuid >= 0xB &&
		    (reg[1] & 0xFF) != 0) { // Check if Extended Topology
					    // Information is available

			// Populate Hybrid Status (CPUID 7.EDX[15]) for Alder
			// Lake+
			cpuid(0x7, 0, &reg[0], &reg[1], &reg[2], &reg[3]);
			if (reg[3] & (1 << 15)) {
				cpuid_info.topology.is_hybrid = 1;
				cpuid_info.topology.pcore_count = 1; // We have
								     // at least
								     // 1 P-Core
								     // as BSP
			...
			if (cpuid_info.flags.htt) {
				if (((cpuid_info.proc_info.raw >> 16) & 0xFF) >
				    (uint32_t)cpuid_info.topology.core_count) {
					cpuid_info.topology.thread_count *= 2;
				} else {
					cpuid_info.flags.htt =
						!cpuid_info.flags.htt;
				}
			}
