...
static void lfb8_put_char(int row, int col, uint8_t ch, uint8_t attr)
{
	...
	uint8_t *pixel_row = (uint8_t *)lfb_base +
			     row * FONT_HEIGHT * lfb_stride + col * FONT_WIDTH;
...
static void lfb16_put_char(int row, int col, uint8_t ch, uint8_t attr)
{
	...
	uint16_t *pixel_row = (uint16_t *)lfb_base +
			      row * FONT_HEIGHT * lfb_stride + col * FONT_WIDTH;
	for (int y = 0; y < FONT_HEIGHT; y++) {
		uint8_t font_row = font_data[ch][y];
		for (int x = 0; x < FONT_WIDTH; x++) {
...
static void lfb24_put_char(int row, int col, uint8_t ch, uint8_t attr)
{
	...
	uint8_t *pixel_row = (uint8_t *)lfb_base +
			     row * FONT_HEIGHT * lfb_stride +
			     col * FONT_WIDTH * 3;
	for (int y = 0; y < FONT_HEIGHT; y++) {
		uint8_t font_row = font_data[ch][y];
		for (int x = 0; x < FONT_WIDTH * 3; x += 3) {
			uint32_t colour = font_row & 0x80 ? fg_colour :
							    bg_colour;
			pixel_row[x + 0] = colour & 0xff;
			colour >>= 8;
			pixel_row[x + 1] = colour & 0xff;
			colour >>= 8;
...
void screen_init(void)
{
	...
	if (use_lfb) {
		...
		// Clip the framebuffer size to make sure we can map it into the
		// 0.5GB device region. This will produce a garbled display, but
		// that's better than nothing.
		if (lfb_stride > 32768) {
			lfb_stride = 32768;
			if (lfb_width >
			    (int)(lfb_stride / lfb_bytes_per_pixel)) {
				lfb_width =
					(int)(lfb_stride / lfb_bytes_per_pixel);
			}
		}
		...
		for (int y = 0; y < lfb_height; y++) {
			for (int x = 0; x < (lfb_width / pixels_per_word);
			     x++) {
				line[x] = 0;
			}
		...
		for (int i = 0; i < 16; i++) {
			uint32_t r = ((vga_pallete[i].r * r_max) / 255)
				     << screen_info->red_pos;
			uint32_t g = ((vga_pallete[i].g * g_max) / 255)
				     << screen_info->green_pos;
			uint32_t b = ((vga_pallete[i].b * b_max) / 255)
...
void scroll_screen_region(int start_row, int start_col, int end_row,
			  int end_col)
{
	if (start_row < 0)
		start_row = 0;
...
void save_screen_region(int start_row, int start_col, int end_row, int end_col,
			uint16_t buffer[])
{
	if (start_row < 0)
		start_row = 0;
...
void restore_screen_region(int start_row, int start_col, int end_row,
			   int end_col, const uint16_t buffer[])
{
	if (start_row < 0)
		start_row = 0;
	if (start_col < 0)
...
void print_char(int row, int col, char ch)
{
	...
	put_char(row, col, ch,
		 (current_attr & 0x0f) | (shadow_buffer[row][col].attr & 0xf0));
}
