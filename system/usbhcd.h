...
typedef struct {
	...
	bool (*assign_address)(usb_hcd_r, const usb_hub_t *, int, usb_speed_t,
			       int, usb_ep_t *);
	bool (*configure_hub_ep)(usb_hcd_r, const usb_ep_t *,
				 const usb_hub_t *);
	bool (*configure_uc_ep)(usb_hcd_r, const usb_ep_t *);
	bool (*setup_request)(usb_hcd_r, const usb_ep_t *,
			      const usb_setup_pkt_t *);
	bool (*get_data_request)(usb_hcd_r, const usb_ep_t *,
				 const usb_setup_pkt_t *, const void *, size_t);
 ...
 * Used internally by the various HCI drivers.
 */
static inline void build_setup_packet(usb_setup_pkt_t *pkt, int type,
...
{
	return (size == 8) || ((speed != USB_SPEED_LOW) &&
...
static inline bool valid_usb_device_descriptor(const uint8_t *buffer)
{
	...
	return desc->length == sizeof(usb_device_desc_t) &&
	       desc->type == USB_DESC_DEVICE;
}
...
static inline bool valid_usb_config_descriptor(const uint8_t *buffer)
{
	...
	return desc->length == sizeof(usb_config_desc_t) &&
	       desc->type == USB_DESC_CONFIGURATION;
}
...
bool find_attached_usb_keyboards(const usb_hcd_t *hcd, const usb_hub_t *hub,
				 ...
				 usb_ep_t keyboards[], int max_keyboards,
				 int *num_keyboards);
