...
#define tty_full_redraw() tty_send_region(0, 0, 24, 79);

#define tty_partial_redraw()           \
	tty_send_region(1, 34, 5, 79); \
	tty_send_region(7, 0, 8, 79);  \
	if (enable_temperature)        \
		tty_send_region(1, 16, 1, 26);

#define tty_error_redraw() tty_send_region(10, 0, 23, 79);

#define tty_normal() serial_echo_print(TTY_NORMAL);

#define tty_inverse() serial_echo_print(TTY_INVERSE);

#define tty_disable_cursor() serial_echo_print(TTY_DISABLE_CURSOR);

#define tty_clear_screen() serial_echo_print(TTY_CLEAR_SCREEN);
