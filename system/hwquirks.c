...
void quirks_init(void)
{
	...
	if (quirk.root_vid == PCI_VID_INTEL &&
	    quirk.root_did == 0x1130) { // Intel i815
		if (pci_config_read16(0, 0, 0, PCI_SUB_VID_REG) ==
		    PCI_VID_ASUS) { // ASUS
			if (pci_config_read16(0, 0, 0, PCI_SUB_DID_REG) ==
			    0x8027) { // TUSL2-C
	...
	if (quirk.root_vid == PCI_VID_INTEL &&
	    quirk.root_did == 0x6F00) { // Broadwell-E (Xeon-D)
		if (pci_config_read16(0, 0, 0, PCI_SUB_VID_REG) ==
		    PCI_VID_SUPERMICRO) { // Super Micro
