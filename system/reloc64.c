...
{
	Elf64_Addr addr;
	__asm__ __volatile__("leaq _start(%%rip), %0" : "=r"(addr) : : "cc");
	return addr;
}
...
{
	Elf64_Addr offs;
	__asm__ __volatile__("movq _GLOBAL_OFFSET_TABLE_(%%rip), %0"
			     : "=r"(offs)
			     :
			     : "cc");
	return offs;
...
static void get_dynamic_info(Elf64_Dyn *dyn_section, Elf64_Addr load_offs,
...
static void do_relocation(Elf64_Addr load_addr, Elf64_Addr load_offs,
...
static void do_relocations(Elf64_Addr load_addr, Elf64_Addr load_offs,
			   Elf64_Addr rel_addr, Elf64_Addr rel_size)
{
...
void reloc(void)
{
	...
	Elf64_Dyn *dyn_section =
		(Elf64_Dyn *)(load_addr + get_dynamic_section_offset());
	get_dynamic_info(dyn_section, load_offs, dyn_info);

	do_relocations(load_addr, load_offs, dyn_info[DT_RELA]->d_un.d_ptr,
		       dyn_info[DT_RELASZ]->d_un.d_val);
