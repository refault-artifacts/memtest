...
static void determine_imc(void)
{
	...
	if (cpuid_info.vendor_id.str[0] == 'A' &&
		...
		case 0x3:
			...
			break;
		case 0x5:
			imc_type = IMC_K14; // C- / E- / Z- Series APU (Family
					    // 14h)
			break;
		case 0x6:
		...
		case 0xA:
			if (cpuid_info.version.extendedModel == 5) {
				imc_type = IMC_K19_CZN; // AMD Cezanne APU
							// (Model 0x50-5F -
							// Family 19h)
	...
	if (cpuid_info.vendor_id.str[0] == 'G' &&
	    cpuid_info.version.family == 6 &&
	    cpuid_info.version.extendedModel) {
		switch (cpuid_info.version.model) {
		case 0x5:
			switch (cpuid_info.version.extendedModel) {
			case 2:
				imc_type = IMC_NHM; // Core i3/i5 1st Gen 45 nm
						    // (Nehalem/Bloomfield)
				break;
			case 3:
				...
				break;
			case 4:
				imc_type = IMC_HSW_ULT; // Core 4th Gen
							// (Haswell-ULT)
				break;
			case 5:
				imc_type = IMC_SKL_SP; // Skylake/Cascade
						       // Lake/Cooper Lake
						       // (Server)
				break;
			default:
		...
		case 0x6:
			...
			case 3:
				...
				break;
			case 4:
				imc_type = IMC_HSW; // Core 4th Gen (Haswell w/
						    // GT3e)
				break;
			case 5:
		...
		case 0x7:
			...
			case 0x4:
				...
				break;
			case 0x9:
				imc_type = IMC_ADL; // Core 12th Gen (Alder
						    // Lake-P)
				break;
			case 0xA:
				imc_type = IMC_RKL; // Core 11th Gen (Rocket
						    // Lake)
				break;
			case 0xB:
				imc_type = IMC_RPL; // Core 13th Gen (Raptor
						    // Lake)
				break;
			default:
				break;
		...
		case 0xA:
			switch (cpuid_info.version.extendedModel) {
			case 0x1:
				imc_type = IMC_NHM_E; // Core i7 1st Gen 45 nm
						      // (NHME)
				break;
			case 0x2:
				imc_type = IMC_SNB; // Core 2nd Gen (Sandy
						    // Bridge)
				break;
			case 0x3:
				...
				break;
			case 0x6:
				imc_type = IMC_ICL_SP; // Ice Lake-SP/DE
						       // (Server)
				break;
			case 0x9:
				imc_type = IMC_ADL; // Core 12th Gen (Alder
		...
		case 0xC:
			...
			case 0x1:
				...
				break;
			case 0x2:
				imc_type = IMC_WMR; // Core i7 1st Gen 32 nm
						    // (Westmere)
				break;
			case 0x3:
				...
				break;
			case 0x8:
				imc_type = IMC_TGL; // Core 11th Gen (Tiger
						    // Lake-U)
				break;
			default:
				break;
		...
		case 0xD:
			switch (cpuid_info.version.extendedModel) {
			case 0x2:
				imc_type = IMC_SNB_E; // Core 2nd Gen (Sandy
						      // Bridge-E)
				break;
			case 0x7:
		...
		case 0xE:
			switch (cpuid_info.version.extendedModel) {
			case 0x1:
				imc_type = IMC_NHM; // Core i7 1st Gen 45 nm
						    // (Nehalem/Bloomfield)
				break;
			case 0x2:
				imc_type = IMC_SNB_E; // Core 2nd Gen (Sandy
				...
				break;
			case 0x3:
				imc_type = IMC_IVB_E; // Core 3rd Gen (Ivy
						      // Bridge-E)
				break;
			case 0x4:
				imc_type = IMC_SKL_UY; // Core 6th Gen (Sky
						       // Lake-U/Y)
				break;
			case 0x5:
				imc_type = IMC_SKL; // Core 6th Gen (Sky
						    // Lake-S/H/E)
				break;
			case 0x7:
				...
				break;
			case 0x8:
				imc_type = IMC_KBL_UY; // Core 7/8/9th Gen
						       // (Kaby/Coffee/Comet/Amber
						       // Lake-U/Y)
				break;
			case 0x9:
		...
		case 0xF:
			switch (cpuid_info.version.extendedModel) {
			case 0x3:
				imc_type = IMC_HSW_E; // Core 3rd Gen
						      // (Haswell-E)
				break;
			case 0x4:
...
static void determine_cpu_model(void)
{
	...
	// The brand string is not available so we need to figure out CPU what
	// we have.
	switch (cpuid_info.vendor_id.str[0]) {
	case 'A':
		// AMD Processors
		...
		case 6:
			...
			case 7:
				cpu_model = "AMD Duron";
				// Duron stepping 0 CPUID for L2 is broken (AMD
				// errata T13)
			...
			break;
		default:
			// All AMD family values >= 10 have the Brand ID feature
			// so we don't need to find the CPU type.
			break;
		}
		break;
...
static uint32_t memspeed(uintptr_t src, uint32_t len, int iter)
{
	...
	run_time_clk =
		((len * iter) / (double)run_time_clk) * clks_per_msec * 2;
...
static void measure_memory_bandwidth(void)
{
	...
	// Locate enough free space for tests. We require the space to be mapped
	// into our virtual address space, which limits us to the first 2GB.
	for (int i = 0; i < pm_map_size && pm_map[i].start < VM_PINNED_SIZE;
	     i++) {
		...
		if (try_start < BENCH_MIN_START_ADR) {
			if ((pm_map[i].end << PAGE_SHIFT) >=
			    (BENCH_MIN_START_ADR + mem_test_len * 2)) {
		...
		// Avoid the memory region where the program is currently
		// located.
		if (try_start < (uintptr_t)_end &&
		    try_end > (uintptr_t)_start) {
			try_start = (uintptr_t)_end;
			try_end = try_start + mem_test_len * 2;
		}

		uintptr_t end_limit = (pm_map[i].end < VM_PINNED_SIZE ?
					       pm_map[i].end :
					       VM_PINNED_SIZE)
				      << PAGE_SHIFT;
	...
	if (l1_cache) {
		l1_cache_speed =
			memspeed(bench_start_adr, (l1_cache / 3) * 1024, 50);
	}

	// Measure L2 BW using half the L2 cache size
	if (l2_cache) {
		l2_cache_speed =
			memspeed(bench_start_adr, l2_cache / 2 * 1024, 50);
	}

	// Measure L3 BW using half the L3 cache size
	if (l3_cache) {
		l3_cache_speed =
			memspeed(bench_start_adr, l3_cache / 2 * 1024, 50);
	}
