...
#define RSDTSignature \
	('R' | ('S' << 8) | ('D' << 16) | ('T' << 24)) // Root System
						       // Description Table

#define XSDTSignature \
	('X' | ('S' << 8) | ('D' << 16) | ('T' << 24)) // Extended System
						       // Description Table

#define MADTSignature \
	('A' | ('P' << 8) | ('I' << 16) | ('C' << 24)) // Multiple APIC
						       // Description Table

#define FADTSignature \
	('F' | ('A' << 8) | ('C' << 16) | ('P' << 24)) // Fixed ACPI Description
						       // Table

#define HPETSignature \
	('H' | ('P' << 8) | ('E' << 16) | ('T' << 24)) // High Precision Event
						       // Timer

#define EINJSignature \
	('E' | ('I' << 8) | ('N' << 16) | ('J' << 24)) // Error Injection Table
#define ERSTSignature \
	('E' | ('R' << 8) | ('S' << 16) | ('T' << 24)) // Error Record
						       // Serialization Table
#define CPEPSignature \
	('C' | ('P' << 8) | ('E' << 16) | ('P' << 24)) // Corrected Platform
						       // Error Polling Table
#define HESTSignature \
	('H' | ('E' << 8) | ('S' << 16) | ('T' << 24)) // Hardware Error Source
						       // Table

#define SLITSignature \
	('S' | ('L' << 8) | ('I' << 16) | ('T' << 24)) // System Locality
						       // Information Table
						       // (NUMA)
#define SRATSignature                     \
	('S' | ('R' << 8) | ('A' << 16) | \
	 ('T' << 24)) // System Resource Affinity Table (NUMA)
...
static const efi_guid_t EFI_ACPI_1_RDSP_GUID = { 0xeb9d2d30,
						 0x2d88,
						 0x11d3,
						 { 0x9a, 0x16, 0x00, 0x90, 0x27,
						   0x3f, 0xc1, 0x4d } };
static const efi_guid_t EFI_ACPI_2_RDSP_GUID = { 0x8868e871,
						 0xe4f1,
						 0x11d3,
						 { 0xbc, 0x22, 0x00, 0x80, 0xc7,
						   0x3c, 0x88, 0x81 } };
...
static rsdp_t *scan_for_rsdp(uintptr_t addr, int length)
{
	...
	while (ptr < end) {
		...
		if (*ptr == RSDPSignature1 && *(ptr + 1) == RSDPSignature2 &&
		    acpi_checksum(ptr, 20) == 0) {
			if (rp->revision < 2 ||
			    (rp->length < 1024 &&
			     acpi_checksum(ptr, rp->length) == 0)) {
				return rp;
			}
		}
		ptr += 4;
...
#ifdef __x86_64__
static rsdp_t *
find_rsdp_in_efi64_system_table(efi64_system_table_t *system_table)
{
	...
	for (uint32_t i = 0; i < system_table->num_config_tables; i++) {
		if (memcmp(&config_tables[i].guid, &EFI_ACPI_2_RDSP_GUID,
			   sizeof(efi_guid_t)) == 0) {
			table_addr = config_tables[i].table;
			break;
		}
		if (memcmp(&config_tables[i].guid, &EFI_ACPI_1_RDSP_GUID,
			   sizeof(efi_guid_t)) == 0) {
			table_addr = config_tables[i].table;
		}
	}
...
#else
static rsdp_t *
find_rsdp_in_efi32_system_table(efi32_system_table_t *system_table)
{
...
static uintptr_t find_rsdp(void)
{
	...
	if (efi_info->loader_signature == EFI64_LOADER_SIGNATURE) {
		uintptr_t system_table_addr = (uintptr_t)efi_info->sys_tab_hi
						      << 32 |
					      (uintptr_t)efi_info->sys_tab;
		system_table_addr = map_region(
			system_table_addr, sizeof(efi64_system_table_t), true);
		if (system_table_addr != 0) {
			rp = find_rsdp_in_efi64_system_table(
				(efi64_system_table_t *)system_table_addr);
			if (rp)
				rsdp_source = "EFI64 system table";
		}
	}
#else
	if (efi_info->loader_signature == EFI32_LOADER_SIGNATURE) {
		uintptr_t system_table_addr = map_region(
			efi_info->sys_tab, sizeof(efi32_system_table_t), true);
		if (system_table_addr != 0) {
			rp = find_rsdp_in_efi32_system_table(
				(efi32_system_table_t *)system_table_addr);
			if (rp)
				rsdp_source = "EFI32 system table";
		}
	}
#endif
	if (rp == NULL) {
	...
	if (rp == NULL) {
		...
		if (rp)
			rsdp_source = "BIOS reserved area";
	}
	if (rp == NULL) {
...
static uintptr_t find_acpi_table(uint32_t table_signature)
{
	...
	if (rp->revision >= 2) {
		rt = (rsdt_header_t *)map_region(rp->xsdt_addr,
						 sizeof(rsdt_header_t), true);
		if (rt == NULL) {
			return 0;
		}
		// Validate the XSDT.
		...
		rt = (rsdt_header_t *)map_region(rp->xsdt_addr, rt->length,
						 true);
		...
		uint64_t *tab_ptr =
			(uint64_t *)((uint8_t *)rt + sizeof(rsdt_header_t));
		uint64_t *tab_end = (uint64_t *)((uint8_t *)rt + rt->length);

		while (tab_ptr < tab_end) {
			uintptr_t addr = *tab_ptr++; // read the next table
						     // entry
			uint32_t *ptr = (uint32_t *)map_region(
				addr, sizeof(uint32_t), true);
		...
		rt = (rsdt_header_t *)map_region(rp->rsdt_addr,
						 sizeof(rsdt_header_t), true);
		if (rt == NULL) {
			return 0;
		}
		...
		rt = (rsdt_header_t *)map_region(rp->rsdt_addr, rt->length,
						 true);
		if (rt == NULL || acpi_checksum(rt, rt->length) != 0) {
			return 0;
		}
		...
		uint32_t *tab_ptr =
			(uint32_t *)((uint8_t *)rt + sizeof(rsdt_header_t));
...
static bool parse_fadt(uintptr_t fadt_addr)
{
	...
	if (fadt->length > FADT_MINOR_REV_OFFSET) {
		acpi_config.ver_min =
			*(uint8_t *)(fadt_addr + FADT_MINOR_REV_OFFSET) & 0xF;
	}
	...
	if (fadt->length > FADT_X_PM_TMR_BLK_OFFSET) {
		rt = (acpi_gen_addr_struct *)map_region(
			fadt_addr + FADT_X_PM_TMR_BLK_OFFSET,
			sizeof(acpi_gen_addr_struct), true);
