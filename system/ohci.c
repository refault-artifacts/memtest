...
#define OHCI_CTRL_HCFS_RST \
	0x00000000 // Host Controller Functional State is Reset
#define OHCI_CTRL_HCFS_RES \
	0x00000040 // Host Controller Functional State is Resume
#define OHCI_CTRL_HCFS_RUN 0x00000080 // Host Controller Functional State is Run
#define OHCI_CTRL_HCFS_SUS \
	0x000000c0 // Host Controller Functional State is Suspend
...
{
	// Prepare for host controller setup (see section 5.1.1.3 of the OHCI
	// spec.).
	...
	if ((read32(&op_regs->control) & OHCI_CTRL_HCFS) !=
	    OHCI_CTRL_HCFS_SUS) {
		return false;
	}
...
{
	// The OHCI reset lasts for 10ms, but the USB specification calls for
	// 50ms (but not necessarily continuously). So do it 5 times.
	for (int i = 0; i < 5; i++) {
		write32(&op_regs->rh_port_status[port_idx],
			OHCI_PORT_CONNECT_CHG | OHCI_PORT_RESET_CHG);
		write32(&op_regs->rh_port_status[port_idx],
			OHCI_SET_PORT_RESET);
		if (!wait_until_set(&op_regs->rh_port_status[port_idx],
				    OHCI_PORT_RESET_CHG, 1000 * MILLISEC)) {
			return false;
		}
	}
...
static void build_ohci_td(ohci_td_t *td, uint32_t control, const void *buffer,
			  size_t length)
{
...
static void build_ohci_ed(ohci_ed_t *ed, uint32_t control,
			  const ohci_td_t *head_td, const ohci_td_t *tail_td)
{
	// Set the skip flag before modifying the head and tail pointers, in
	// case we are modifying an active ED. Use write32() to make sure the
	// compiler doesn't reorder the writes.
...
{
	uint32_t ed_speed = (ep->device_speed == USB_SPEED_LOW) ?
				    OHCI_ED_SPD_LOW :
				    OHCI_ED_SPD_FULL;

	uint32_t control = OHCI_ED_FMT_GEN | OHCI_ED_DIR_TD | ed_speed |
			   ep->max_packet_size << 16 | ep->endpoint_num << 7 |
			   ep->device_id;
			  ...
			  const usb_setup_pkt_t *setup_pkt)
{
	...
	build_ohci_td(&ws->td[0],
		      OHCI_TD_DP_SETUP | OHCI_TD_DT_USE_TD | OHCI_TD_DT_0 |
			      OHCI_TD_DI_NO_INT,
		      setup_pkt, sizeof(usb_setup_pkt_t));
	build_ohci_td(&ws->td[1],
		      OHCI_TD_DP_IN | OHCI_TD_DT_USE_TD | OHCI_TD_DT_1 |
			      OHCI_TD_DI_NO_DLY,
		      0, 0);
			     ...
			     const void *buffer, size_t length)
{
	...
	build_ohci_td(&ws->td[1],
		      OHCI_TD_DP_IN | OHCI_TD_DT_USE_TD | OHCI_TD_DT_1 |
			      OHCI_TD_DI_NO_INT,
		      buffer, length);
	build_ohci_td(&ws->td[2],
		      OHCI_TD_DP_OUT | OHCI_TD_DT_USE_TD | OHCI_TD_DT_1 |
			      OHCI_TD_DI_NO_DLY,
		      0, 0);
...
static void poll_keyboards(const usb_hcd_t *hcd)
{
	...
	while (td != NULL) {
		...
		build_ohci_td(td, td->control & ~OHCI_TD_CC, kbd_rpt,
...
static const hcd_methods_t methods = {
	...
	.allocate_slot = NULL,
	.release_slot = NULL,
	.assign_address = assign_usb_address, // use the base implementation for
					      // this method
...
bool ohci_reset(uintptr_t base_addr)
{
	...
	if (read32(&op_regs->control) & OHCI_CTRL_IR) {
		...
		if (!wait_until_clr(&op_regs->control, OHCI_CTRL_IR,
				    1000 * MILLISEC)) {
			return false;
		}
	}

	// Reset the controller, but preserve the frame interval set by the SMM
	// or BIOS.
...
bool ohci_probe(uintptr_t base_addr, usb_hcd_t *hcd)
{
	...
	write32(&op_regs->control,
		OHCI_CTRL_HCFS_RUN | OHCI_CTRL_CLE | OHCI_CTRL_CBSR0);
	flush32(&op_regs->interrupt_status, ~0);

	// Some controllers ignore writes to these registers when in suspend
	// state, so write them now.
	uint32_t max_packet_size = ((frame_interval - 210) * 6) / 7;
	uint32_t frame_interval_toggle =
		(read32(&op_regs->fm_interval) & OHCI_FIT) ^ OHCI_FIT;
	write32(&op_regs->fm_interval,
		frame_interval_toggle | max_packet_size << 16 | frame_interval);
	...
	if (~rh_descriptor_a & OHCI_RHDA_NPS) {
		// If we have individual port power control, clear the port
		// power control mask to allow us to power up all ports at once.
		if (rh_descriptor_a & OHCI_RHDA_PSM) {
			write32(&op_regs->rh_descriptor_b,
				rh_descriptor_b & OHCI_RHDB_DR);
		}
	...
	for (int port_idx = 0; port_idx < root_hub.num_ports; port_idx++) {
		...
		if (~port_status & OHCI_PORT_POWERED)
			continue;

		// Check if anything is connected to this port.
		if (~port_status & OHCI_PORT_CONNECTED)
			continue;

		// Reset the port.
		if (!reset_ohci_port(op_regs, port_idx))
			continue;
		...
		if (~port_status & OHCI_PORT_CONNECTED)
			continue;
		if (~port_status & OHCI_PORT_ENABLED)
			continue;

		// Now the port has been enabled, we can determine the device
		// speed.
		usb_speed_t device_speed = (port_status & OHCI_PORT_LOW_SPEED) ?
						   USB_SPEED_LOW :
						   USB_SPEED_FULL;
		...
		if (find_attached_usb_keyboards(
			    hcd, &root_hub, 1 + port_idx, device_speed,
			    num_devices, &num_devices, keyboards, MAX_KEYBOARDS,
			    &num_keyboards)) {
			continue;
		}
		...
		// port.
		write32(&op_regs->rh_port_status[port_idx],
			OHCI_CLR_PORT_ENABLE);
	}

	print_usb_info(" Found %i device%s, %i keyboard%s", num_devices,
		       num_devices != 1 ? "s" : "", num_keyboards,
		       num_keyboards != 1 ? "s" : "");
	...
	// Initialise the interrupt ED and TD for each keyboard interface and
	...
	for (int kbd_idx = 0; kbd_idx < num_keyboards; kbd_idx++) {
		...
		build_ohci_td(kbd_td,
			      OHCI_TD_DP_IN | OHCI_TD_DT_USE_TD | OHCI_TD_DT_0 |
				      OHCI_TD_DI_NO_DLY,
			      kbd_rpt, sizeof(hid_kbd_rpt_t));
		build_ohci_ed(kbd_ed, ohci_ed_control(kbd), kbd_td + 0,
			      kbd_td + 1);
	...
	write32(&op_regs->control, OHCI_CTRL_HCFS_RUN | OHCI_CTRL_CLE |
					   OHCI_CTRL_PLE | OHCI_CTRL_CBSR0);
	flush32(&op_regs->interrupt_status, ~0);
