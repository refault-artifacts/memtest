...
{
	const heap_t *heap = &heaps[heap_id];
	if (heap->segment >= 0 && mark > pm_map[heap->segment].end &&
	    mark <= heap->end) {
...
void heap_init(void)
{
	// Use the largest 20-bit addressable physical memory segment for the
	// low-memory heap. Use the largest 32-bit addressable physical memory
	// segment for the high-memory heap. Exclude memory occupied by the
	// program or below it in that segment.
	...
	for (int i = 0; i < pm_map_size && pm_map[i].end <= PAGE_C(4, GB);
	     i++) {
		uintptr_t try_heap_start = pm_map[i].start;
		uintptr_t try_heap_end = pm_map[i].end;
		if (program_start >= try_heap_start &&
		    program_end <= try_heap_end) {
