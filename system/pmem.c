...
static int sanitize_e820_map(e820_entry_t new_map[],
			     const e820_entry_t orig_map[], int orig_entries)
{
    ...
    */
	...
	for (int i = 0; i < orig_entries; i++) {
		...
		change_point[chg_idx]->addr =
			orig_map[i].addr + orig_map[i].size;
	...
	while (still_changing) {
		...
		for (int i = 1; i < 2 * orig_entries; i++) {
			// If current_addr > last_addr or if current_addr =
			// last_addr and current is a start addr and last is an
			// end addr, swap.
			if ((change_point[i]->addr <
			     change_point[i - 1]->addr) ||
			    ((change_point[i]->addr ==
			      change_point[i - 1]->addr) &&
			     change_point[i]->start &&
			     !change_point[i - 1]->start)) {
...
static void init_pm_map(const e820_entry_t e820_map[], int e820_entries)
{
	...
	for (int i = 0; i < e820_entries; i++) {
		if (e820_map[i].type == E820_RAM ||
			...
			if (end > RESERVED_MEM_START &&
			    end < RESERVED_MEM_END) {
				end = RESERVED_MEM_START;
			}

			pm_map[pm_map_size].start = (start + PAGE_SIZE - 1) >>
						    PAGE_SHIFT;
			pm_map[pm_map_size].end = end >> PAGE_SHIFT;
			num_pm_pages += pm_map[pm_map_size].end -
					pm_map[pm_map_size].start;
			if ((pm_map_size > 0) &&
			    (pm_map[pm_map_size].start ==
			     pm_map[pm_map_size - 1].end)) {
				pm_map[pm_map_size - 1].end =
					pm_map[pm_map_size].end;
			} else {
				pm_map_size++;
			}
		}
...
{
	// Do an insertion sort on the pm_map. On an already sorted list this
	// should be a O(1) algorithm.
...
void pmem_init(void)
{
	...
	int sanitized_entries = sanitize_e820_map(sanitized_map,
