...
#define EHCI_MAX_PFL_LENGTH \
	1024 // Maximum number of entries in periodic frame list
...
#define EHCI_USBCMD_ITC(n) \
	((n) << 16) // Interrupt Threshold Control = n (n = 1,2,4,8,16,32,64)
...
#define EHCI_LP_TYPE_SITD \
	(2 << 1) // Type is Split Transaction Isochronous Transfer Descriptor
...
#define EHCI_QH_HBPM(n) \
	((n) << 30) // High Bandwidth Pipe Multiplier = n (n = 1..3)
...
// NOTE: The ehci_qtd_t structure supports both the 32-bit and 64-bit variants.
// But we always allocate data buffers in the first 4GB, so we can simply
// initialise the buffer pointer extensions to zero.
typedef volatile struct {
...
static bool reset_host_controller(ehci_op_regs_t *op_regs)
{
	write32(&op_regs->usb_command,
		read32(&op_regs->usb_command) | EHCI_USBCMD_HCR);

	usleep(1 * MILLISEC); // some controllers need time to recover from
			      // reset

	return wait_until_clr(&op_regs->usb_command, EHCI_USBCMD_HCR,
			      1000 * MILLISEC);
}
...
{
	write32(&op_regs->usb_command,
		EHCI_USBCMD_R_S | EHCI_USBCMD_FLS_1024 | EHCI_USBCMD_ITC(8));
	return wait_until_clr(&op_regs->usb_status, EHCI_USBSTS_HCH,
			      1000 * MILLISEC);
}
...
{
	write32(&op_regs->usb_command,
		read32(&op_regs->usb_command) & ~EHCI_USBCMD_R_S);
	return wait_until_set(&op_regs->usb_status, EHCI_USBSTS_HCH,
			      1000 * MILLISEC);
}
...
{
	write32(&op_regs->usb_command,
		read32(&op_regs->usb_command) | EHCI_USBCMD_PSE);
}

static void enable_async_schedule(ehci_op_regs_t *op_regs)
{
	write32(&op_regs->usb_command,
		read32(&op_regs->usb_command) | EHCI_USBCMD_ASE);
}
...
static bool disable_async_schedule(ehci_op_regs_t *op_regs)
{
	write32(&op_regs->usb_command,
		read32(&op_regs->usb_command) & ~EHCI_USBCMD_ASE);
	return wait_until_clr(&op_regs->usb_status, EHCI_USBSTS_ASS,
			      1000 * MILLISEC);
}
...
static bool reset_ehci_port(ehci_op_regs_t *op_regs, int port_idx)
{
	...
	return wait_until_clr(&op_regs->port_sc[port_idx], EHCI_PORT_SC_PR,
			      5 * MILLISEC);
}
...
static void disable_ehci_port(ehci_op_regs_t *op_regs, int port_idx)
{
	...
	(void)wait_until_clr(&op_regs->port_sc[port_idx], EHCI_PORT_SC_PED,
			     1000 * MILLISEC);
}
...
static void build_ehci_qtd(ehci_qtd_t *this_qtd, const ehci_qtd_t *final_qtd,
			   uint8_t control, uint16_t dt, const void *buffer,
			   size_t length)
{
...
static void build_ehci_qhd(ehci_qhd_t *qhd, const ehci_qtd_t *qtd,
			   const usb_ep_t *ep, bool is_interrupt)
{
	...
	qhd->epcc[0] = ep->max_packet_size << 16 | EHCI_QH_DTC(1) |
		       endpoint_speed << 12 | ep->endpoint_num << 8 |
		       ep->device_id << 0;

	qhd->epcc[1] = EHCI_QH_HBPM(1) | parent_port << 23 | parent_addr << 16;
...
static bool do_async_transfer(const workspace_t *ws, int num_tds)
{
	...
	for (int td_idx = 0; td_idx < num_tds; td_idx++) {
		...
		if (qtd->status &
		    (EHCI_QTD_HALTED | EHCI_QTD_DB_ERR | EHCI_QTD_BABBLE |
		     EHCI_QTD_TR_ERR | EHCI_QTD_MMF | EHCI_QTD_PS)) {
			ok = false;
			break;
		}
	}
			   ...
			   int device_id, usb_ep_t *ep0)
{
	...
	if (!assign_usb_address(hcd, hub, port_num, device_speed, device_id,
				ep0)) {
		return false;
	}
			  ...
			  const usb_setup_pkt_t *setup_pkt)
{
	...
	build_ehci_qtd(&ws->qtd[0], &ws->qtd[1], EHCI_QTD_PID_SETUP,
		       EHCI_QTD_DT(0), setup_pkt, sizeof(usb_setup_pkt_t));
	build_ehci_qtd(&ws->qtd[1], &ws->qtd[1], EHCI_QTD_PID_IN,
		       EHCI_QTD_DT(1), 0, 0);
			     ...
			     const void *buffer, size_t length)
{
	...
	build_ehci_qtd(&ws->qtd[0], &ws->qtd[2], EHCI_QTD_PID_SETUP,
		       EHCI_QTD_DT(0), setup_pkt, sizeof(usb_setup_pkt_t));
	build_ehci_qtd(&ws->qtd[1], &ws->qtd[2], EHCI_QTD_PID_IN,
		       EHCI_QTD_DT(1), buffer, length);
	build_ehci_qtd(&ws->qtd[2], &ws->qtd[2], EHCI_QTD_PID_OUT,
		       EHCI_QTD_DT(1), 0, 0);
	build_ehci_qhd(&ws->qhd[0], &ws->qtd[0], ep, false);
...
static void poll_keyboards(const usb_hcd_t *hcd)
{
	...
	for (int kbd_idx = 0; kbd_idx < ws->num_keyboards; kbd_idx++) {
		...
		uint8_t error_mask = EHCI_QTD_HALTED | EHCI_QTD_DB_ERR |
				     EHCI_QTD_BABBLE | EHCI_QTD_TR_ERR |
				     EHCI_QTD_MMF | EHCI_QTD_PS;
		if (~status & error_mask) {
			hid_kbd_rpt_t *prev_kbd_rpt =
				&ws->prev_kbd_rpt[kbd_idx];
			if (process_usb_keyboard_report(hcd, kbd_rpt,
							prev_kbd_rpt)) {
		...
		build_ehci_qtd(kbd_qtd, kbd_qtd, EHCI_QTD_PID_IN, dt, kbd_rpt,
			       sizeof(hid_kbd_rpt_t));
...
bool ehci_reset(int bus, int dev, int func, uintptr_t base_addr)
{
	...
	while (ext_cap_ptr != 0) {
		uint8_t ext_cap_id =
			pci_config_read8(bus, dev, func, ext_cap_ptr + 0);
		if (ext_cap_id == EHCI_EXT_CAP_OS_HANDOFF) {
			...
			while (pci_config_read8(bus, dev, func,
						ext_cap_ptr + 2) &
			       1) {
				if (timer == 0)
					return false;
	...
	ehci_op_regs_t *op_regs =
		(ehci_op_regs_t *)(base_addr + cap_regs->cap_length);
...
bool ehci_probe(uintptr_t base_addr, usb_hcd_t *hcd)
{
	...
	// Allocate and initialise a periodic frame list. This needs to be
	// aligned on a 4K page boundary. Some controllers don't support a
	// programmable list length, so we just use the default length.
	uintptr_t pfl_addr = heap_alloc(HEAP_TYPE_LM_1,
					EHCI_MAX_PFL_LENGTH * sizeof(uint32_t),
					PAGE_SIZE);
	if (pfl_addr == 0) {
	...
	if (hcs_params & EHCI_HCS_PPC) {
		...
		for (int port_idx = 0; port_idx < root_hub.num_ports;
		     port_idx++) {
			uint32_t port_status =
				read32(&op_regs->port_sc[port_idx]);
			if (~port_status & EHCI_PORT_SC_PP) {
				flush32(&op_regs->port_sc[port_idx],
					port_status | EHCI_PORT_SC_PP);
				port_power_changed = true;
			}
		}
		if (port_power_changed) {
			usleep(20 * MILLISEC); // EHCI maximum port power-up
					       // time
		}
	}
	...
	for (int port_idx = 0; port_idx < root_hub.num_ports; port_idx++) {
		...
		if (num_keyboards >= MAX_KEYBOARDS)
			break;
		...
		if (~port_status & EHCI_PORT_SC_CCS)
			continue;
		...
		// Reset the port.
		if (!reset_ehci_port(op_regs, port_idx))
			continue;
		...
		// Look for keyboards attached directly or indirectly to this
		// port.
		if (find_attached_usb_keyboards(
			    hcd, &root_hub, 1 + port_idx, USB_SPEED_HIGH,
			    num_hs_devices, &num_hs_devices, keyboards,
			    MAX_KEYBOARDS, &num_keyboards)) {
			continue;
		}

		// If we didn't find any keyboard interfaces, we can disable the
		// port.
		disable_ehci_port(op_regs, port_idx);
	}

	print_usb_info(
		" Found %i low/full speed device%s, %i high speed device%s, %i keyboard%s",
		num_ls_devices, num_ls_devices != 1 ? "s" : "", num_hs_devices,
		num_hs_devices != 1 ? "s" : "", num_keyboards,
		num_keyboards != 1 ? "s" : "");
	if (num_ls_devices > 0 && i_have_companions) {
		print_usb_info(
			" Handed over low/full speed devices to companion controllers");
	}
	...
	// Initialise the interrupt QHD and QTD for each keyboard interface and
	...
	for (int kbd_idx = 0; kbd_idx < num_keyboards; kbd_idx++) {
		...
		build_ehci_qtd(kbd_qtd, kbd_qtd, EHCI_QTD_PID_IN,
			       EHCI_QTD_DT(0), kbd_rpt, sizeof(hid_kbd_rpt_t));
