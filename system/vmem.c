...
// space, using 2MB pages, initially identity mapped to the first 4GB of
// physical memory. We use the third GB to map the physical memory window we are
// currently testing, and the following 512MB to map the screen frame buffer,
// ACPI tables, and any hardware devices we need to access that are not in the
// permanently mapped regions.
...
uintptr_t map_region(uintptr_t base_addr, size_t size, bool only_for_startup)
{
	...
	// Check if the requested region is permanently mapped. If it is only
	// needed during startup, this includes the region we will eventually
	// use for the memory test window.
	if (last_addr <
		    (only_for_startup ? VM_REGION_START : VM_WINDOW_START) ||
	    (base_addr > VM_REGION_END && last_addr <= VM_SPACE_END)) {
		return base_addr;
	}
	...
	while (curr_virt_page < device_pages_used &&
	       curr_phys_page <= last_phys_page) {
		uintptr_t mapped_phys_page = pd3[curr_virt_page++] >>
					     VM_PAGE_SHIFT;
	...
	// If not, map it. Note that this will extend a partial match at the end
	// of the current map.
	while (curr_phys_page <= last_phys_page) {
		if (device_pages_used == MAX_REGION_PAGES)
			return 0;
		pd3[device_pages_used++] =
			(curr_phys_page++ << VM_PAGE_SHIFT) + 0x83;
	}
	// Reload the PDBR to flush any remnants of the old mapping.
	load_pdbr();
	// Return the mapped address.
	return VM_REGION_START + first_virt_page * VM_PAGE_SIZE +
	       base_addr % VM_PAGE_SIZE;
}

bool map_window_uncached(uintptr_t phys, uintptr_t npages, bool flush)
{
    //print_usb_mirror("phys: 0x%016x, npages: 0x%08x", phys, npages);
    uintptr_t ppn = phys >> VM_PAGE_SHIFT;

	for (uintptr_t i = 0; i < npages; i++) {
		pd2[i] = ((ppn + i) << VM_PAGE_SHIFT) + 0x9B;
	}
	if (flush) {
		// Reload the PDBR to flush any remnants of the old mapping.
		load_pdbr();
